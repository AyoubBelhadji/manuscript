...
\usepackage{bbm}
%\usepackage{natbib}
\usepackage[backend=bibtex,natbib=true,  maxnames=3,style = authoryear,isbn=false,url=false,doi=false,eprint=false]{biblatex}
...
Once an embedding is defined, one can make use of the abundant tools of linear operators such as linear algebra or functional analysis to resolve the geometrical problem that raised form the sub-sampling problem. Under this perspective, we can explain the recourse to random sub-sampling in many problems with linear structure: non asymptotic random matrix theory offers strong tools to tackle these geometric problems in a universal way. A universality that lacks to deterministic sub-sampling approaches.
 ...
 % these linear spaces, the task of looking for representatives boils down to approximating a sum of matrices or operators using a smaller sum; and randomized averaging was extensively used for its versatility and its strong theoretical guarantees.


% working with kernels combines well with spectral approaches and This approach is valid for discrete and continuous sets 


% For example, an element of a discrete set can be represented by a vector living in finite dimensional vector space, while an element of a continuous set can be represented by a function living in a Hilbert space. 


% These embeddings are usually associated to kernels.



% would capture the essential of the information avoiding any unnecessary redundancy.


% One way to set up this strategy is to look for a linear representation of the set and to make use of the abundant tools of linear algebra and functional analysis.  In these linear spaces, the task of looking for representatives boils down to approximating a sum of matrices or operators using a smaller sum; and randomized averaging was extensively used for its versatility and its strong theoretical guarantees. 
...
% the most widely used geometric quantity to quantify the quality of representer sub-sampling 


% these randomized sub-sampling techniques were defined using 

% first-order informations, that are the distanc


% independent random variables that may coincide making the sampling non optimal. One may expect to improve the sub-sampling efficiency by using a randomized averaging that avoids redundancy and capture the essential of the information.
...
% Many approximation tasks in applied mathematics can be reformulated as problems of estimations under \emph{size constraints}, that is the estimation of some object based on a partial knowledge of the available data; this object can be a scalar, a vector, a matrix or even a function. 
% These approximations are conducted, mainly, for three purposes: improving the numerical complexity of an existing algorithm (\emph{numerical issue}), reducing the cost of labelling (\emph{sensing issue}) or making an algorithm more interpretable (\emph{interpretation issue}); and have many applications in data analysis, signal processing, machine learning and statistics: continuous signal discretization, numerical integration, dimension reduction, learning on budget, preconditioning...
...
% This thesis studies determinantal point processes as a universal tool for subsampling. 
 ...
 % Determinantal Point Processes (DPPs) were introduced by \cite{Mac75} as probabilistic models for beams of fermions in quantum optics. Since then, DPPs have been thoroughly studied in random matrix theory \parencite{Joh05}, and have more recently been adopted in machine learning \citep*{KuTa12}, spatial statistics \citep*{LaMoRu15}, and Monte Carlo methods \parencite{BaHa16}.
...
% \subsection{Independent sampling: from linear regression to kernel interpolation}
...
In some situations, calculating $\bm{w}^{*}$ using a pseudo-inverse formula is not affordable, either because $N$ is very large and the computation resources are not sufficient (the numerical issue), or because obtaining the $N$ labels $(y_{n})_{n \in [N]}$ is expensive (the sensing issue). In such a situation, looking for representatives can be beneficial.
 ...
 % corresponds to a family of indices $(n_{i})_{i \in I}$ and a family of weights $(s_{i})_{i \in I}$ $...$
 ...
 $\bm{S}_{j,i_{j}}  = s_{j}>0$, and the other elements of $\bm{S}$ are equal to $0$. The constant $c$ represents the \emph{budget} of sub-sampling,
  % that contains only one non-vanishing element in a row: for every $n \in [N]$, if $\bm{S}_{i,n}  = s_{i}>0$ then the $n$-element of $[N]$ is kept in the subset $S$, and if $\bm{S}_{i,n} = 0$ then the $n$-element of $[N]$ is removed.
  ...
  % The values of the $s_{n}$ corresponds to either the multiplicities of these rows, or some rescaling factors necessary for the theoretical analysis. In particular $\bm{S}^{\Tran} \bm{X}$ is the sub-matrix of $\bm{X}$ with the indices of the rows 
...
% An example of such a result is given in the following.
...
Beside vanilla linear regression, this reduction can be beneficial in other problems with a linear structure. For example, for the problem of graph sparsification, i.e. sub-sampling the edges of a graph while maintaining its spectral properties. For this problem, the dimension $d$ is the number of the nodes, which can be very small compared to the number of the edges $N$. Leverage score sampling was studied for this problem under the name of \emph{effective resistance sampling} \citep*{SpTe04,SpSr11}. Sub-sampling nodes in a graph can also be beneficial, e.g. for the recovery of band-limited signals \citep*{PuTrGrVa18}. In this setting, $d$ is the  size of the band of the signal and $N$ is the number of the nodes. Finally, the notion of leverage score sampling was used for low-rank approximations, of general matrices \citep*{DrMaMu07}, as we shall see more in detail in Chapter~\ref{chapter:cssp}.
 ...
 % have a significant impact when $N$ is very large  sub-sampling using the leverage score distribution leads to compressing the 


% To take into account all these situations, a non uniform sub-sampling have to be used. It corresponds to the vector $\bm{p}$ defined by
% \begin{equation}
% \forall n \in [N], \:\: p_{n} =  \ell_{n} = \frac{1}{d}\|\tilde{x}_{n}\|^{2}.
% \end{equation}



% This result is widely known under the name of \emph{Matrix Bernstein concentration inequality} because it is the natural extension of the Bernstein concentration inequality.

% However, the fluctuations of $I_{c}$ around $\mathbb{I}_{d}$, expressed by the matrix variance of the sum $\|\EX \sum_{i \in [c]} \big( (z_{i}z_{i}^{\Tran}/c - \mathbb{I}_{d})^{2} \big)\|_{2}$, would be large if some $\tilde{x}_{n}$ have large norms compared to the other vectors. 

% We give in the following an example of a matrix concentration inequality. 

% the  matrix variance of the sum defined by $\|\frac{1}{c}\sum_{i \in [c]} z_{i}z_{i}^{\Tran} - \mathbb{I}_{d}\|_{2}$


 

 % would be preferable to include them with a probability that scales as their renorm

% In particular, $\mathbb{I}_{d}$ can be seen as the expected value of a random matrix. 
...
%   repsentatives $S \subset [N]$ of the set of observations and approximate $\bm{w}^{*}$ using only the informations associated to the subset $S$: the subset of the rows $\bm{X}_{S,:}$ and the subset of the labels $\bm{y}_{S}$. For example, in \citep*{DrMaMu06}, the following approximation was proposed
% \begin{equation}
% \bm{w}_{S}^{*} = (\bm{S}^{\Tran}\bm{X})^{+} (\bm{S}^{\Tran}\bm{y}),
% \end{equation}
% where $\bm{S} \in \mathbb{R}^{|S| \times N}$ is the sampling matrix defined by

% \begin{equation}
% \bm{S}_{i,j}
% \end{equation}

%  contains zeros except defined by $\bm{S}_{i,j} = 1$ if $j \in S$ and  $\bm{D}$ is a diagonal matrix that corresponds to rescaling the rows of $\bm{X}_{S,:}$ and $\bm{y}_{S}$. In particular, they proved that, if $S$ is ...., and $\bm{D}_{i,i} = 1/\sqrt{p_{i}}$, then 
% \begin{equation}
...
% The vectors $x_{n}$ may have different interpretation depending on the context of the problem. For example, in the context of Graph sparsification the $x_{n}$ represent the edges and $d$ is the number of vertices and $\bm{M}$ is the Laplacian matrix.
...
% Moreover, it can be extended to kernel matrix approximations under the name of \emph{ridge-leverage scores} distribution.
...
Another field of application of matrix sub-sampling is low-rank approximation of kernel matrices. These approximations are widely used to scale up kernel methods~\parencite{ScSm18,ShCr04}. To give an example, consider the kernel ridge regression problem, defined for
 % that are recurrently used in kernel methods \parencite{Bac13,AlMa15}. For example,
...
% Hence, the need to  can be used such as
% \begin{equation}
% \bm{\alpha}_{S}^{*} = 
% \end{equation}
...
\end{equation}
% In particular, the minimal budget to conducted the Nystrom approximation scales as $\mathcal{O}(d_{\eff}(\lambda) \log d_{\eff}(\lambda))$:
The effective dimension plays the same role as the dimension $d$ for the sparsification of the identity matrix $\mathbb{I}_{d}$ reviewed around Theorem~\ref{thm:matrix_bernstein}: the minimal  budget $|S|$ has to scale as $\mathcal{O}(d_{\eff}(\lambda) \log d_{\eff}(\lambda))$ to obtain a statistical risk within a factor $(1+\epsilon)$ of the statistical risk obtained when the full matrix $\bm{K}(\bm{x})$ is used.
 ...
 % Moreover, tMoreover, i.i.d. sampling requires an $\Omega(d_{\eff}(\lambda) \log d_{\eff}(\lambda)$ samples, where $d_{\eff}$ is the effective dimension 
...
\end{equation}
% where the \emph{nodes} $x_{n}$ are taken to be random in $\X$.
...
%defined by
 % $| \int_{\X} f(x)g(x)\mathrm{d}\omega(x) - \sum_{n \in N}w_{n}f(x_{n}) |$ of a function $f$ that belongs to the unit ball of the RKHS $\F$. 

% \begin{equation}
% \sup\limits_{\substack{\|f\|_{\F} \leq 1}} \Big| \int_{\X} f(x)g(x)\mathrm{d}\omega(x) - \sum_{n \in N}w_{n}f(x_{n}) \Big|.
% \end{equation}

% As we can see, both the \emph{nodes} $x_{n}$ and the $\emph{weights}$ $w_{n}$ 
%  this problem appears in quadrature problems of functions that belong to reproducing kernel Hilbert spaces. 
...
% In a nutshell, this result implies that for a given $\lambda >0$, error level\footnote{It is a regularization parameter.} $\lambda$
...
We continue on this direction on Chapter~\ref{chap:dppkq} and Chapter~\ref{chap:CVS} where we try to solve the problem of the intractability of the continuous ridge leverage score density using DPPs and one of its variants, continuous volume sampling. We mention that the contributions of these two chapters are extensions of the work initiated in Chapter~\ref{chapter:cssp} on the column subset selection problem. In particular, we investigated in this chapter volume sampling through the lens of DPPs. Before giving a detailed account of these contributions, we review, in the following section, the existing work on the volume sampling distribution and some of its applications to subsampling problems with a linear structure.
 ...
 % DPP or volume sampling that we introduce in te following section
% Moreover, many typical notions of matrix sub-sampling were showed to extend to this problem. For example, there exists a distribution $q_{*}$ called the continuous ridge leverage score distribution for which 
% \begin{equation}
% \|f - \sum\limits_{n \in [N]} \frac{w_{n}}{q_{*}(x_{n})^{1/2}}k(x_{n},.) \|_{\F}^{2} \leq \lambda, 
% \end{equation}
% whenever $N \geq C d_{\eff}(\lambda) \log d_{\eff}(\lambda)$, with $d_{\eff}(\lambda)$ is the effective degree of freedom defined by
% \begin{equation}
% d_{\eff}(\lambda) = \Tr \bm{\Sigma}(\bm{\Sigma} + N\lambda \mathbb{I}_{\F})^{-1}.
% \end{equation}
...
% \end{figure}
 ...
 % For example, in the kernel ridge regression problem, the $x_{n}$ are fixed and it is required to sub-sample from the set $\{ x_{1}, \dots, x_{N} \}$. 




% \subsubsection{Matrix sparsification}
% The problem of matrix sparsification is at the heart of many applications in data analysis and machine learning: pre-conditioning of linear systems (solvers) \parencite{Golub1965}, subsampling for linear regression \citep*{DrMaMu06}, low rank approximations \citep*{DrMaMu07}, low-rank kernel matrix approximations \parencite{Bac13,AlMa15}, linear systems on graphs \citep*{SpTe04,SpSr11} and signal recovery on graphs \citep*{PuTrGrVa18}. All these problems start with an s.d.p matrix that writes
% \begin{equation}
% \bm{M} = \sum\limits_{n \in [N]} x_{n}x_{n}^{\Tran},
% \end{equation}
% and build up a random matrix 
...
%Volume sampling distribution favours subset of rows that complementarity among the rows of the selected subset. 
...
In particular the case $c = d$ corresponds to the volume sampling distribution. These distributions satisfy the unbiasedness property~\eqref{eq:first_mention_unbiasedness}, as shown by \cite{DeWa17}. However, the empirical investigation conducted in~\parencite{DeWaHs18} shows that dual volume sampling in the regime $c = \Omega(d \log d)$ can lead to an inferior approximation compared to leverage score sampling. In other words, dual volume sampling is better used in the "minimalist sampling" regime $c \approx d$, and leverage score sampling is better used in "Bernstein" regime $c = \Omega(d \log d)$.
 ...
 % that we present in details in Chapter~\ref{chapter:dpp},
...
% This geometric intuition is based on the notion of principal angles between subspaces
%  that we exploit to extend these results to random designs on continuous domain. 

% there is no way to compare  

% comparing volume sampling and leverage scores sampling is 
% The interesting aspect about this bound is that it does not have an equivalent under independent sampling. Indeed, the number of the sub-sampled rows is equal to $d$ which is lower than the typical $\mathcal{O}(d \log d)$ budget of independent sampling. 


% Observe that under volume sampling distribution, the budget of sampling is minimal 
% The interesting aspect about this distribution is the size of the budege


% Moreover, they proved the unbiasedness property~\eqref{eq:first_mention_unbiasedness} for a larger class of distributions called \emph{dual volume sampling} distributions. These distributions charge positively subsets $I \subset [N]$ of fixed cardinality $c \geq d$ as following
% \begin{equation}\label{eq:second_mention_volume_sampling}
% \Prb(Y = I) \propto \Det \bm{X}_{I}^{\Tran} \bm{X}_{I}.
% \end{equation} 


% , and we start by a result from the nineties.

% \begin{theorem}[\cite{BeTe90}]
% Let $\bm{X} \in \mathbb{R}^{N \times d}$ of full rank with $N \geq d$. Then
% \begin{equation}
% \bm{X}^{+}\bm{y} = \sum\limits_{\substack{I \subset [N]\\|I|=d}} \frac{\Delta_{I}}{\Delta} \bm{X}_{I,:}^{+}\bm{y}_{I},
% \end{equation}
% where $\Delta_{I} = \Det^{2} \bm{X}_{I,:}$ and $\Delta = \sum\limits_{\substack{I \subset [N]\\ |I|=d}} \Delta_{I}$.
% % $w_{I}^{*}$ be the solution of the linear system
% \end{theorem}
...
From a technical point of view, we harness the geometric intuitions developed in Chapter~\ref{chapter:cssp} to work out the theoretical analysis of these quadratures. In particular, we make use of the notion of principal angles between subspaces to derive a tractable upper bound of the expected value of the squared interpolation error
\begin{equation}
\EX_{\DPP} \|\mu_{g} - \bm{\Pi}_{\mathcal{T}(\bm{x})} \mu_{g}\|_{\mathcal{F}}^{2},
\end{equation}
which has no tractable formula.

The material of this chapter
is based on the following article

\begin{itemize}
\item \fullcite{BeBaCh19}.
\end{itemize}
...
In Chapter~\ref{chap:CVS}, we continue the line of research initiated in Chapter~\ref{chapter:cssp} and Chapter~\ref{chap:dppkq}, and we study kernel quadrature and kernel interpolation based on nodes that follow the  continuous volume sampling distribution, a generalization of discrete volume sampling. Contrary to the projection DPP of Chapter~\ref{chap:dppkq}, we show that the expected value of the squared interpolation error has a closed formula under the distribution of continuous volume sampling
\begin{equation}
\EX_{\mathrm{CVS}} \|\mu_{g} - \bm{\Pi}_{\mathcal{T}(\bm{x})} \mu_{g}\|_{\F}^{2} = \sum\limits_{m \in \mathbb{N}^{*}} \langle g,e_{m} \rangle_{\mathrm{d}\omega}^{2} \epsilon_{m}(N),
\end{equation}
where
\begin{equation}
...
Figure~\ref{fig:introduction_VS_th_vs_exp} illustrates the comparison between the theoretical values of the $\epsilon_{m}(N))$ and their estimation using the MCMC sampler of \parencite{ReGh19} in the case of the periodic Sobolev space of order $s = 2$.
 % under the distribution of the continuous volume sampling ....
...
% Since then, DPPs have been thoroughly studied in random matrix theory \parencite{Joh05}, and have more recently been adopted in machine learning \citep*{KuTa12}, spatial statistics \citep*{LaMoRu15}, and Monte Carlo methods \parencite{BaHa16}.
...
% The space of $\sigma$-finite Borel measures on $\Omega$, denoted $M(\Omega)$, is a complete separable metric space if and only if $\Omega$ is a complete separable metric space \parencite{Par05}??.
...
\end{equation}
where $m \in \mathbb{N}^{*}$, $B_{1}, \dots, B_{m}$ are relatively compact open subsets of $\mathcal{D}$ and $r_{1}, \dots, r_{m} \in \mathbb{R}_{+}$; see Lemma 3.1.1 in \parencite{ScWe08}.
...
\end{equation}
is an example  of an element of $\mathcal{M}(\mathcal{D})$; see Lemma 3.1.2 in \parencite{ScWe08}.
...
% \mu(B) \in \{0,1\}.
% \end{equation}
Another example is given by the set of simple counting measures Lemma 3.1.4 in \parencite{ScWe08}
\begin{equation}
...
Now, if an element $x_{n}$ is repeated more than once in the collection, $\gamma$ is no longer simple and it can be seen as a discrete subset of $\mathcal{D}$ with multiplicities. This intuition is based on this result; see Proposition 9.1.III in \parencite{DaVe07II}.
...
A Poisson point process is simple if and only if its intensity measure is \emph{diffuse}, i.e. it has no atoms (Lemma 3.2.1 in \parencite{ScWe08})
\begin{equation}
...
Note that the Campbell theorem can be applied for general point processes: the term $\sum_{x \in \gamma} f(x)$ should be replaced by $\int_{\mathcal{D}} f \gamma$. Moreover, it can be extended to functions $f: \mathcal{D} \rightarrow \mathbb{R}$ that are integrable with respect to $\gamma_1$. See Theorem 1.2.5 in \parencite{BaBlKa20} for a general statement and its proof.
...
\end{equation}
See Proposition 2.3.25 in \parencite{BaBlKa20}.
...
\begin{proposition}[Lemma 17 in \parencite{HoKrPeVi06}]\label{prop:projection_DPP_cardinality}
...
\begin{theorem}[Theorem 7 in \parencite{HoKrPeVi06}]\label{thm:mixture_of_projection_DPPs}
...
The second recurrent relaxation is the Hermitianity of $\kappa$: there are examples of DPPs associated to non-Hermitian kernels ; see Section 2.5 in \parencite{Sos00} and \parencite{Bru18}. Yet, many interesting aspects of DPPs require this condition: negative correlations, tractable numerical simulation $\dots$
...
This class of DPPs was the topic of intense research recently for machine learning applications; we refer the reader to \parencite{KuTa12} for details. In this setting, the domain is taken to be  $\mathcal{D} = [d]$ and usually; the reference measure is the counting measure $\mathrm{d}\omega = \sum\limits_{i \in [d]} \delta_{i}$; and the definition~\ref{def:def_dpp_chapter_2} is equivalent to the following.
...
%Note that it is also common to see in the literature a related definition called an $\bm{L}$-ensemble \citet{KuTa12}.
 % it is not obvious that \eqref{eq:def_dpp} consistently defines a point process. One ; see \parencite{Mac75} and \parencite{Sos00}[Theorem 3].
...
The spherical ensemble is another example of a DPP based on the eigenvalues of random matrices. This ensemble is defined on the complex plan; yet, using the inverse of the stereographic projection, it can be seen as a DPP on the sphere $\mathbb{S}^{2}$, hence the name. This ensemble was already used in theoretical physics for the modeling of repulsive particles in the sphere \parencite{Cai81} \parencite{FoJaMa92}. The spectral representation of this DPP was discovered by \cite{Kri09}. 



\begin{theorem}[Theorem 3 in \parencite{Kri09}]\label{thm:spherical_ensemble}
...
We illustrate this technique in Section~\ref{sec:Gaussian_quadrature} in Chapter~\ref{chap:dppkq}. We refer to \parencite{GaBaVa20} for more details on this topic.
...
\subsubsection{The HKPV algorithm for projection DPPs}
 A universal sampling  algorithm of projection DPPs was proposed by \nparencite*{HoKrPeVi06}. The idea behind the algorithm goes as follow.
...
% \end{align*}
\begin{theorem}[Proposition 19 in \parencite{HoKrPeVi06}]
...
In general, sampling directly from $f_{\kappa,n}$ is not possible. In \parencite{BaHa16} proposed to use rejection sampling based on the following observation 
 \begin{equation}
...
\end{equation}
Indeed, by Bernstein's inequality~\parencite{Lor83}, we have
\begin{equation}
...
of $\bm{\Sigma}_{\kappa}$ is not tractable. For this reason, many approximate samplers were proposed for DPPs to circumvent sampling the mixture. 
 ...
 MCMC algorithms were proposed in \parencite{AnGhRe16} and \parencite{GaBaVa17} for DPPs and $k$-DPPs defined in discrete domains. These algorithms do not require the spectral decomposition of the kernel matrix. An extension of the sampler of \parencite{AnGhRe16} to continuous domains was proposed in \parencite{ReGh19}. We will present and discuss this algorithm in Chapter~\ref{chap:CVS}. Recently, a fast sampler of intractable OPE was proposed in \parencite{GaBaVa20}. This algorithm is based on an MCMC in the domain of tri-diagonal matrices; see Section~\ref{sec:Gaussian_quadrature}. 
...
This rationale corresponds to a rich literature on randomized algorithms for column subset selection \parencite{DeVe06,DrMaMu07,BoDrMI11}. A prototypal example corresponds to sampling $s$ columns of $\bm{X}$ i.i.d. from a multinomial distribution of parameter $\bm{p} \in \mathbb{R}^{d}$. This parameter $\bm{p}$ can be the squared norms of each column \parencite{DFKVV04}, for instance, or the more subtle $k$-leverage scores \parencite{DrMaMu07}. While the former only takes $\mathcal{O}(d N)$ time to evaluate, it comes with loose guarantees; see Section~\ref{subsec:length_square_sampling}. The $k$-leverage scores are more expensive to evaluate, since they call for a truncated SVD of order $k$, but they come with tight bounds on the ratio of their expected approximation error over that of PCA.
...
% For example, in \cite{DFKVV04} the authors proved additive approximation bound while \parencite{DrMaMu07} proved relative approxi, \parencite{BoDrMI11}, \parencite{DeVe06} and \parencite{PaKyBo14}
% Types of bounds
...
% DPPs can be viewed as the kernel machine of point processes; they were introduced by \cite{Mac75} in quantum optics, and their use widely spread after the 2000s in random matrix theory \parencite{Joh05}, machine learning \parencite{KuTa12}, spatial statistics \parencite{LaMoRu15}, and Monte Carlo methods \parencite{BaHa16}, among others. 
...
%% \end{definition}
%
%% The main application of the CSSP is features selection \parencite{BoMaDr08}. Consider for example $\bm{X} \in \mathbb{R}^{N \times d}$ a matrix representing $N$ objects with $d$ features, the output of a CSSP algorithm would be a subset of $k$ features represented by a matrix $\bm{C} \in \mathbb{R}^{N \times k}$. These $k$ features are actual features from the data as opposed to the $k$ eigenvectors returned by the Principal Component Analysis that are linear combinations of all the $d$ features.
%\begin{equation}
%    \| \bm{X} - \Pi_{k}\bm{X}\|_{\nu} \leq \| \bm{X} - \Pi_{C}\bm{X}\|_{\nu} \leq \gamma \| \bm{X} - \Pi_{k}\bm{X}\|_{\nu}.
%\end{equation}
%
%
%Note that while we focus on the problem of selecting exactly $k$ columns from $\bm{X}$, another possible relaxation of the CSSP problem was studied in the literature \parencite{DrMaMu07}, \parencite{BoDrMI11}, \parencite{DeVe06} and \parencite{PaKyBo14}: pick a subset of $c$ columns (with $c >k$) of $\bm{X}$ called $\bm{C}$ such that
%\begin{equation}
...
%
%We present a new randomized algorithm for the problem of $k$-CSSP such that the $k$ columns selected are as diverse as possible. This randomized diversity could be expressed naturally using Determinantal Point Processes (DPPs). These models were introduced by Macchi \parencite{Mac75} in quantum physics and their use spread: random matrix theory \parencite{Joh05}, machine learning \parencite{KuTa12} and Monte Carlo methods \parencite{BaHa16}. We can define a DPP on $[d]$ using a positive semi-definite matrix $\bm{K}~\in~\mathbb{R}^{d\times d}$.
...
% We refer to \cite[Chapter 5: Orthogonalization and least squares, Algorithm: Householder with column pivoting]{GoVa96}  for a recall on the Pivoted QR decomposition. We recall here a RRQR algorithm proposed in \parencite{GuEi96} and used latter in \parencite{BoMaDr09} in the deterministic phase of DoublePhase algorithm (see the end of this section). We call this algorithm $\Algo{StrongRRQR}$. It was proven in \parencite{GuEi96} that $\Algo{StrongRRQR}$ interchanges any pair of columns that sufficiently increases $\Det(\bm{R}_{11})$.
...
% }
% }
% \caption{Pseudocode of a strong rank revealing QR decomposition. \parencite{GuEi96} Algorithm 4.}
...
In practice, a RRQR factorization algorithm interchanges pairs of columns and updates or builds a QR decomposition on the fly.
 % start with a QR decomposition of the matrix $\bm{X}$, then apply a column permutation on $\bm{X}$ and update the decomposition. It was proven in \parencite{GuEi96} that $\Algo{StrongRRQR}$ interchanges any pair of columns that sufficiently increases $\Det(\bm{R}_{11})$.
...
\centering
 ...
 \hline
 Pivoted QR & $2^{k}\sqrt{d-k}$ & $\mathcal{O}(dNk)$ & \parencite{GoVa96}\\
 \hline
 Strong RRQR (Alg. 3) & $\sqrt{(d-k) k +1}$ & not polynomial & \parencite{GuEi96} \\
 \hline
 Strong RRQR (Alg. 4) & $\sqrt{f^{2}(d-k) k +1}$ & $\mathcal{O}(dNk \log_{f}(d))$ & \parencite{GuEi96}\\
 \hline
\end{tabular}
...
% We should note here that the lower bound in \parencite{DeVe06} is for general matrices; however, the upper bound of Theorem 3 in \parencite{PaKyBo14}  suppose that the leverage scores follows a power law decay.
...
Later, sampling according to \eqref{e:vs} was shown to be doable in polynomial time \parencite{DeRa10}. Using a worst case example, \cite{DRVW06} proved that the $k+1$ factor in \eqref{e:vsBoundFr} cannot be improved.
...
%We refer to \parencite{BoMaDr09} for details.
...
% \end{equation}
% Thus $\hat{\bm{w}}_{S} = (\bm{X}\bm{S})^{+}\bm{y}^{*}$, and $\bm{w}_{S} = \bm{S}(\bm{X}\bm{S})^{+}\bm{y}^{*}$. In \parencite{LiHa18}, the authors proved bounds for the estimation error of the estimator $\bm{w}_{S}$.
...
% This result was proven using the identity \parencite{Sla18}
% \begin{equation}
...
% %   Orthogonal Matching Pursuit is a greedy selection algorithm proposed first in \parencite{PaReKr93} and \parencite{DaMaAv97} for sparse atomic decomposition for a signal ($\bm{w}$ using our notations) accessible using $N$ measures ($\bm{y}$) for signal processing applications. The algorithm aims to recover the support of $\bm{w}$, the subset of non vanishing elements $S \subset [d]$, in order to recover $\bm{w}$ by the resolution of a linear problem latter. In this context, the matrix $\bm{X}$ is to be "designed" by the practionner. Such matrices are often structured. This structure was leveraged in \parencite{Tro04} to conduct the first analysis of convergence of OMP, in the noiseless regime ($\nu = 0$), for matrices $\bm{X}$ that satisfy low mutual coherence condition: for such  matrices the constraint $N = \Omega(s^{2})$ is required in order to recover the support using OMP. This rate was improved in \parencite{TrGi07} to $N = \Omega(s \log d)$ for a large family of random matrices families that include the Gaussian random matrices or the Bernoulli random matrices. Finally, it was established in \parencite{DaWa10} that the Restricted Isometry Property (RIP) is sufficient with practically the same rate $N = \Omega(s \log d)$.
% % % For example, $\bm{X}$ could be ... as in []  or ... as in []

% %   Now in the noisy regime, additionnaly to support recovery guarantees one would prove upper bounds on the excess risk. See \parencite{SoGuJaNe18} for a modern review on upper bounds for the excess risk for OMP. Typically, the excess risk scales as $\displaystyle \mathcal{O}(\frac{s}{N} \nu)$.

% %   Now in the context of regression for machine learning application, the design of the matrix $\bm{X}$ is fixed. The proven bounds in \parencite{Zha11} still apply under the condition that the matrix $\bm{X}$ satisfy the Restricted Isometry Property or the Nullspace Property. In general, evaluating the RIP or NSP for a matrix $\bm{X}$ is NP-Hard \parencite{BaDoMiSa12} \parencite{TiPf13}.

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \rev{
% Orthogonal Matching Pursuit is a greedy selection algorithm proposed first in \parencite{PaReKr93} and \parencite{DaMaAv97} for sparse atomic decomposition in signal processing. The algorithm aims to recover the support of $\bm{w}$, i.e., the subset of non vanishing elements $S \subset [d]$.
...
% \rev{Now, while in signal processing there is some freedom to choose the matrix $\bm{X}$ to satisfy conditions like RIP, machine learning applications usually consider fixed designs. In general, checking RIP for a matrix $\bm{X}$ is NP-Hard \parencite{BaDoMiSa12,TiPf13}. Therefore, it might be difficult to guarantee the validity of the aforementioned excess risk bounds for OMP in applications where $\bm{X}$ is given.}
...
% In \parencite{BoMaDr09}, the authors consider
% This interpretation given in \parencite{BoMaDr09},
...
% discrete determinantal point processes (DPPs) and the related $k$-DPPs, of which volume sampling is an example. DPPs were introduced by \cite{Mac75} as probabilistic models for beams of fermions in quantum optics. Since then, DPPs have been thoroughly studied in random matrix theory \parencite{Joh05}, and have more recently been adopted in machine learning \citep*{KuTa12}, spatial statistics \citep*{LaMoRu15}, and Monte Carlo methods \parencite{BaHa16}.

% \subsection{Definitions}
% For all the definitions in this section, we refer the reader to \parencite{KuTa12}. Recall that $[d] = \{1,\dots,d\}$.% A point process on $[d]$ is a probability measure over subsets of $[d]$.
% \begin{definition}[DPP]
% Let $\bm{K} \in \mathbb{R}^{d\times d}$ be a positive semi-definite matrix.
% A random subset $Y \subseteq [d]$ is drawn from a DPP of marginal kernel $\bm{K}$ if and only if
% \begin{equation}\label{eq:def_dpp}
...
% %In this paper, we use a definition of DPPs through their ...
% %Note that it is also common to see in the literature a related definition called an $\bm{L}$-ensemble \citet{KuTa12}.
% For a given matrix $\bm{K}$, it is not obvious that \eqref{eq:def_dpp} consistently defines a point process. One sufficient condition is that $\bm{K}$ is symmetric and its spectrum is in $[0,1]$; see \parencite{Mac75} and \parencite{Sos00}[Theorem 3]. In particular, when the spectrum of $\bm{K}$ is included in $\{0,1\}$, we call $\bm{K}$ a projection kernel and the corresponding DPP a \emph{projection} DPP\footnote{All projection DPPs in this paper have symmetric kernels}. Letting $r$ be the number of unit eigenvalues of its kernel, samples from a projection DPP have fixed cardinality $r$ with probability 1 \cite*[Lemma 17]{HoKrPeVi06}.
...
\begin{figure}[!ht]
  \centering
%  \subfigure[SVD]{
    \input{img/cssp/principalangles/principal_angles_3D.tex}
...
The difference between volume sampling and the DPP with kernel $\bm K$ is also manifested by the sampling procedure. Indeed, as we have mentioned, volume sampling is a mixture of projection DPPs corresponding to the kernels $\bm{K}_{T}$. Therefore, given the matrix $\bm{V}$, volume sampling requires to sample the set of eigenvectors $T$; while the projection DPP of kernel $\bm{K}$ does not need this intermediate step. The difference is depicted in Figure~\ref{f:sampling}.
 ...
 % with a graphical depiction in Figure~\ref{f:sampling} of the two procedures to sample from them that we introduced in Section~\ref{subsec:sampling_from_a_dpp}. Figure~\ref{f:sampling} is another illustration of the decomposition of volume sampling as a mixture of projection DPPs.
...
As we have seen in Chapter~\ref{chapter:dpp}, sampling from a projection DPP is possible through the HKPV algorithm.
 % that we recall a variant in Figure~\ref{alg:DPP_SAMPLER}. 
% See also \cite[Theorem 2.3]{KuTa12} for a proof of this variant. 
...
% For a survey of other variants   and . 


% The only nontrivial step is sampling from a projection DPP, for which we give pseudocode in Figure~\ref{alg:DPP_SAMPLER}; see \cite[Theorem 7]{HoKrPeVi06} or \cite[Theorem 2.3]{KuTa12} for a proof. 



% For our purposes, it is enough to remark that general DPPs are mixtures of projection DPPs of different ranks, and that the cardinality of a general DPP is a sum of independent Bernoulli random variables.
...
%  \subfigure[SVD]{
     ...
     % \\
     % \subfloat[Sampling $k$ columns according to VS and our DPP]{\input{img/cssp/colorful_svd_3.tex}}
  %  \subfloat[]{\input{img/sampling_algorithm_extended.tex}}\\
    %\caption{DPP sampling algorithm
    %\label{f:dpp_sampling_algorithm}
    %}
...
% Recall that for a given $S \subset [d]$ a bound of the prediction error $\mathcal{E}(\bm{w}_{S})$ was derived in \parencite{LiHa18}.
...
%, or $\mathcal{O}(dk^{3})$ using the implementation in \parencite{KuTa12}.
...
% Every sample requires to run step 2 in Figure~\ref{alg:DPP_SAMPLER} where the sampling of the set $T$ of singular values using Algorithm 7 in \parencite{KuTa12} runs in $\mathcal{O}(rk)$ operations, followed by a sample from a projection DPP of marginal kernel $\bm{V}_{:,T}^{\phantom{\Tran}}\bm{V}_{:,T}^{\Tran}$ in $\mathcal{O}(dk^{2})$.
...
% Another potential application of our theoretical results is the analysis of signal reconstruction on graphs under 

% A consequence for graph signal reconstruction
...
\end{equation}
where $\ell_{n,\bm{x}}$ are the Lagrange polynomial  \footnote{ At that time, Newton considered divided differences interpolation polynomial \parencite{BuFa97}.} basis polynomials defined by \parencite{BuFa97}
\begin{equation}
...
\end{equation}
and by the Strong Law of Large Numbers, $ \sum_{n \in [N]} f(x_{n})/N$ converges almost surely to $\int_{\mathcal{X}} f(x)g(x) \mathrm{d}\omega(x)$. Moreover, if $ \int_{\mathcal{X}} f(x)^{2} \mathrm{d}\omega(x) < +\infty$, the variance of $\sum_{n \in [N]} f(x_{n})/N$ scales as $\mathcal{O}(1/N^{2})$, and the estimator satisfies a central limit theorem \parencite{RoCa04}. 
...
As we have seen in the introduction, the Newton-Cotes formula is based on equi-distanced nodes on $[a,b]$ and it is exact for polynomials of order smaller than $N-1$ when $N$ nodes are used. Gauss \parencite{Gau1815} observed that equi-distance nodes are not optimal, in a sense that will be defined later, and shed the light on the question of designing the quadrature nodes. This observation gave birth to Gaussian quadrature.
...
He proved that for any positive integer $N$ there exists a unique configuration $\bm{x}_{G}$, also called the \emph{Gauss nodes}, of cardinality $N$ such that $M(\bm{x}_{G}) = 2N-1$. He also proved that $2N-1$ is the maximal order that could be achieved by any configuration of cardinality $N$. His proof \parencite{Gau1815} is based on arguments from continued fractions theory \parencite{Khi97}.
...
measures $\mathrm{d}\omega(t) = \omega(t)\mathrm{d}t$ for arbitrary weight functions $\omega$ and his approach is generalizable to even a broader class of measures \parencite{Sze39}. Typically, $\mathrm{d}\omega$ is assumed to have finite moments of all orders, so that the orthonormal polynomials with respect to $\mathrm{d}\omega$ are well defined by
\begin{align}
...
% \end{equation}
called the \emph{Christoffel numbers}, satisfy the identity \parencite{Sho29} (see also Theorem 3.4.2 in \parencite{Sze39}): 
...
\end{equation}
The roots of the $P_{N}$ are the eigenvalues of the tridiagonal matrix $J_{N}$, and for every eigenvalue $x$ the corresponding eigenvector is the vector $\big(P_{0}(x), \dots, P_{N-1}(x) \big)^{\Tran}$; see Theorem 2.13 in \parencite{GoMe09}. In other words, the spectral decomposition of the Jacobi matrix gives the nodes and the weights of the Gaussian quadrature. The matrix $J_{N}$ is known explicitly for many classic orthogonal polynomials such as Chebyshev, Legendre, Hermite... Building up this matrices amount to the calculation of the moments of the measure $\mathrm{d}\omega$ which is a non trivial task for a general measure $\mathrm{d}\omega$. We refer to \parencite{GoMe09} for more details on the algorithmic construction of Gaussian quadrature.
...
\EX \tilde{J}_N = J_N.
\end{equation}
It is well known that the eigenvalues $x_{1}, \dots, x_{N}$ of $\tilde{J}_{N}$, seen as a vector of $\mathbb{R}^{d}$, have the following density \parencite{DuEd05} with respect to the Lebesgue measure of $\mathbb{R}^{d}$
...
This case is far from being anecdotal. Indeed, this probabilistic relaxation is possible for Gaussian quadrature of other measures \parencite{DuEd02} \parencite{KiNe04}, and the resulting point process coincides with a projection DPP; see \parencite{GaBaVa20} for details on this topic.
...
Finally, there were many attempts to extend Gaussian quadrature to high dimensional domains using common zeros of multidimensional orthogonal polynomials; see \parencite{Xu94} for details on this topic. For example, the construction of a high dimensional Gaussian quadrature would require to work with block Jacobi matrices instead of tridiagonal Jacobi matrices \parencite{Xu94b}. However, this line of research is not developed yet from an algorithmic point of view.
...
\end{equation}
for every Riemann integrable function $f: \mathcal{X} \rightarrow \mathbb{R}$; see Theorem 3.3 in \parencite{DiPi10}. An equivalent characterization, yet more practical to use, of uniformly distributed modulo one sequences is given by the \emph{Weyl criterion}.
...
This is to be compared to the star discrepancy for witch no tractable formula is known: the computation of the star discrepancy is an NP-hard problem \parencite{GnSrWi09}. We shall give, in Section~\ref{sec:kernel_quadrature_framework}, an interpretation of \eqref{eq:ltwo_discrepancy_formula} in the context of kernel quadrature.
...
\end{equation}
Then by Proposition 3.32 in \parencite{DiPi10}
\begin{equation}
...
\end{equation}
These are the Halton sequences \parencite{Hal64}. Another possible construction is the Hammersley configuration defined for a fixed value of $N$:
\begin{equation}
...
have square-integrable partial derivatives $\frac{\partial^{u_{1}+ \dots + u_{d}}f}{\partial x_{1}^{u_{1}} \dots \partial x_{d}^{u_{d}}}$ for every $(u_{1}, \dots,u_{d})~\in~\{0, \dots, s\}^{d}$, then the QMC rule based on Higher-order digital nets converge faster as $\mathcal{O}(\log(N)^{2sd} N^{-2s})$ \parencite{DiPi14}[Theorem 5]. However, the algorithmic construction of these sequences is very technical and we refer to~\parencite{DiPi10} for details on these constructions.
...
As it was mentioned in the introduction of this section, this review is limited to the fundamental aspects of Quasi-Monte Carlo methods relevant for later discussions, and many topics have been left out. We refer to \parencite{DiPi14} for more details on the topic. 
...
In a nutshell, the Nevai class contains measures that have three-term recurrence coefficients converging to the corresponding coefficients of $\mathrm{d}\tilde{\omega}^{*}$.
 ...
 This class contains, at least, all measures that are absolutely continuous with respect to the Lebesgue measure with strictly positive densities; see Section 1.4 of \parencite{Sim10} for details. 
...
\begin{theorem}[Theorem 3 in \parencite{BaHa16}]
...
\end{equation}
can be interpreted as a semi-norm in a Hilbert space that reflects the regularity of the integrand $f$. Indeed, the authors proved the inequality [Proposition 1 in \parencite{BaHa16}]
...
% Before we review the main result of \parencite{BaHa16}, we recall the definition of the Nevai class of measures in the following.
...
\begin{proposition}\label{prop:dirichlet_estimator} [Proposition 3.1, \parencite{CoMaAm20} ]
...
\begin{theorem}\label{thm:dirichlet_quadrature}
...
\end{equation}
The kernel $k_{s}$ can be expressed in closed form using Bernoulli polynomials \parencite{Wah90}
\begin{equation}
...
\end{equation}
Figure~\ref{fig:periodic_sobolev_kernel} illustrates the graphs of the kernel translates $x \mapsto k_{s}(0.3,x)$ for $s \in \{1, 2,3\}$. The elements of $\mathcal{S}_{s}$ are characterized by their Fourier series coefficients (see Example 19, Chapter 7 in \parencite{BeTh11})
\begin{equation}
...
In \cite{Sun05}, the author extended Theorem~\ref{thm:Mercer_for_compact} to $\mathcal{X} = \cup_{i \in \mathbb{N}} \mathcal{X}_{i}$, with the $\mathcal{X}_{i}$s are compacts and $\mathrm{d}\omega(\mathcal{X}_{i})<\infty$. One can also extend Mercer's theorem under a \textit{compact embedding} assumption \parencite{StSc12}: the RKHS $\mathcal{F}$ associated to $k$ is said to be compactly embedded in $\mathbb{L}_{2}(\mathrm{d}\omega)$ if the operator
\begin{align*}
  I_{\mathcal{F}}: \mathcal{F}&\longrightarrow \mathbb{L}_{2}(\mathrm{d}\omega) \\
  f &\longmapsto f
\end{align*}
is compact. This is guaranteed by Assumption~\ref{hyp:the_diagonal_k_is_integrable} (Lemma 2.3, \parencite{StSc12}).

%  that is,
% \begin{equation}\label{eq:condition_integrability_diagonal}
% \int_{\mathcal{X}} k(x,x) \mathrm{d}\omega(x) < \infty.
% \end{equation}
% Note that this condition is not necessary (Example 2.9, \parencite{StSc12}). 
Now, under the compact embedding assumption, the pointwise convergence of the Mercer decomposition to the kernel $k$ is equivalent to the injectivity of the embedding $I_{\mathcal{F}}$ (Theorem 3.1, \parencite{StSc12}). This is guaranteed by Assumption~\ref{hyp:domega_is_nondegenerate} (Chapter 4 in \parencite{StCh08}). 
...
% : the eigenvalues $\sigma_{N+1}$ decreases in plateaus and the size of each plateau corresponds to the multiplicity of the respective eigenvalues. Moreover, the rate $\mathcal{O}(\log^{2s(d-1)} N^{-2s})$ matches the asymptotic behaviour of $\sigma_{N}$ for $N \rightarrow +\infty$.
...
The optimal kernel quadrature $(\bm{x},\hat{\bm{w}})$ has an interpretation within \emph{Bayesian Quadrature} introduced by \parencite{Lar72}. This method aims to quantifies the uncertainty of numerical integration; see \parencite{Dia88} for a historical review of this method.
...
\end{equation}
is Gaussian with 
%as it was shown in \parencite{HuDu12}
...
In other words, the optimal kernel quadrature $(\bm{x},\hat{\bm{w}})$ controls both the mean and the variance of $\int_{\X}f(x)g(x)\mathrm{d}\omega(x)$. This link was leveraged in~\parencite{BOGOS2019} to establish rates of posterior contraction of Bayesian quadrature based on rates of convergence of optimal kernel quadrature for some specific kernels.
...
\cite{Boj81} proved that, for $g=1$, the interpolation of $\mu_{g}$ using the uniform grid over $\X = [0,1]$ has an error in $\mathcal{O}(N^{-2s})$ if $\F$ is the periodic Sobolev space of order $s$, and that any set of nodes leads to that rate at least. A similar rate was proved for $g$ not constant \parencite{NoUlWo15} even though it is only asymptotically optimal in that case.
% , is optimal\rb{What do you mean by optimal at this stage?} for the  , with a convergence rate in $\mathcal{O}(N^{-2s})$. If $g$ is not constant, the uniform grid is only optimal for large values of $N$  but the convergence rate remains $\mathcal{O}(N^{-2s})$ \parencite{NoUlWo15}.

% In the quasi-Monte Carlo (QMC) literature, several designs were investigated for $\X = [0,1]^{d}$, $g = 1$ and $\F$ that may not even be a Hilbert space; see \parencite{DiPi10}. In this context, the term \emph{QMC quadrature rule} means a low discrepancy sequence, loosely speaking a ``well-spread" set of nodes, along with uniform weights $w_{i} \equiv 1/N$. If $\F$ is a Korobov space of order $s \geq 1$, the Halton sequence of nodes \parencite{Hal64} leads to $\mathcal{E}(\mu_1; \bm{x}, (1/N))^2$ in $\mathcal{O}(\log(N)^{2d} N^{-2})$ and higher-order digital nets converge faster as $\mathcal{O}(\log(N)^{2sd} N^{-2s})$ \parencite{DiPi14}[Theorem 5].
...
Scattered data approximation \parencite{Wend04} is another field where quantitative error bounds for kernel interpolation on $\X\subset\mathbb{R}^d$ are investigated; see \parencite{ScWe06} for a modern review. In a few words, these bounds typically depend on quantities such as the \emph{fill-in distance} $\varphi(\bm{x}) = \sup_{y \in \X} \min_{i \in [N]}\|y-x_{i}\|_{2}$,
...
Beside the hypercube, optimal kernel quadrature has been considered on the hypersphere equipped with the uniform measure \parencite{EhGrCh19}, or on $\mathbb{R}^{d}$ equipped with the Gaussian measure \parencite{KaSa19}. In these works, the design construction is adhoc for the space $\X$ and
...
Sequential algorithms offer another approach to designing a configuration of nodes $\bm{x} = \{x_{1}, \dots, x_{N}\}$ for optimal kernel quadrature. For example, in \parencite{HuDu12}, the authors proposed a greedy minimization approach called \emph{Sequential Bayesian quadrature}: based on a configuration $\bm{x}_{t}$ look for $x^{*} \in \X$ such that
...
This approach is reminiscent of Frank-Wolfe algorithm and its different variants \parencite{BaLaOb12,BrOaGiOs15}. In particular, \emph{kernel herding} \parencite{ChWeSm10} corresponds to greedily minimizing
\begin{equation}\label{eq:herding_step}
\|\mu_{g} - \frac{1}{N}\sum\limits_{n \in [N]}k(x_{n},.)\|_{\F}^{2}.
\end{equation}
 This leads to a fast rate in $\mathcal{O}(1/N)$, but only when the integrand is in a finite-dimensional RKHS \parencite{BaLaOb12}. 

 In general, implementing any of these algorithms corresponds to solving several non-convex problems such as \eqref{eq:sbq_step} or \eqref{eq:herding_step}, with many local minima \parencite{HiOe16,Oett17}. We will discuss more in details sequential algorithms in Chapter~\ref{chap:CVS}.
%  that is to select the nodes $x_{1}$ 

% Many algorithms are based 


% Then, the weights were chosen to minimize the posterior variance of the integral of $f$. If the kernel of the Gaussian process is chosen to be $k$, this amounts to minimizing the RHS of \eqref{eq:integral_bound_mean_element}. The case of the Gaussian reference measure was later investigated in detail \parencite{Hag91}, while parametric integrands were considered in \parencite{Min00}.

\subsubsection{Ridge leverage score quadrature}
\label{subsec:okq_algebraic_paradigm}
The regularization of the problem~\eqref{eq:wce_repeated_optimization} offers an alternative approach for the design of the configuration $\bm{x}$ as it was shown in \parencite{Bac17}.
...
Since the publication of \parencite{BeBaCh19}, we obtained a refinement of Theorem~\ref{thm:main_theorem} that we give in the following.
...
\end{equation}
when $N= N_0$.
This compares favourably with the quadrature based on herding with uniform weights that comes with a rate in $\mathcal{O}(\frac{1}{N})$  \parencite{ChWeSm10,BaLaOb12}. 
...
Another advantage of DPPs is that they can be sampled exactly. Because of the orthonormality of $(\psi_n)$, one can write the chain rule for \eqref{eq:pdpp_definition}; see \parencite{HoKrPeVi06}. Sampling each conditional in turn, using e.g. rejection sampling \parencite{RoCa04}, then yields an exact sampling algorithm. Rejection sampling aside, the cost of this algorithm is cubic in $N$ without further assumptions on the kernel. The main limitation is the availability of good proposals for the successive rejection sampling routines.
...
% , where   is the ....   analysis of the  error  that the interpolation error $\|\mu_{g} - \bm{\Pi}_{\mathcal{T}(\bm{x})} \mu_{g}\|^{2}_{\mathcal{F}}$ is the residual of 
...
\begin{equation}\label{eq:cos_ratio_det_eq_1}
...
% Let $\mathrm{d}\omega$ be the uniform measure on $\mathcal{X} = [0,1]$, and let the RKHS kernel be \parencite{BeTh11}
% \begin{equation}
% k_{s}(x,y) = 1+ 2\sum\limits_{m \in \mathbb{N}^{*}} \frac{1}{m^{2s}} \cos(2\pi m(x-y)),
% \end{equation}
...
% The quantity~\eqref{eq:surrogate_worst_inerpolation_error} is an approximation of the worst interpolation error. Indeed, if $g = \sum\limits_{\delta \in [\Delta]} \epsilon_{\delta} e_{\delta}$ for some $(\epsilon_{1}, \dots, \epsilon_{\Delta})$, then $\|g\|_{\mathrm{d}\omega} = 1$.
...
% $\mathrm{d}\omega$ to be the Gaussian measure on $\mathcal{X} = \mathbb{R}$ along with the RKHS kernel $\displaystyle k_{\gamma}(x,y) = \exp[-(x-y)^{2}/2\gamma^{2}]$. 
...
% is a close runner-up to SBQ and definitely takes the lead for large enough $N$.
...
Now, compared to other algorithms and quadratures, DPPKQ seem to give consistent empirical performance: the squared interpolation error scales as $\mathcal{O}(\sigma_{N})$ after a stagnation at the plateau of the initial error. 
 ...
 % This distribution is not  In particular, in the ridgeless regime the continuous ridge leverage score distribution is not defined.  
...
% An easier route than de-symmetrization could be to replace the product in \eqref{eq:max_error_as_function_of_cos} by a sum, but this is beyond the scope of this article.
 ...
 % these rates are not optimal for , and they are almost optimal for RKHSs with infinite 
...
% we take the expectation of the bound in Proposition~\ref{prop:kernel_perturbation_inequality} under the proposed DPP. 
%to Lemma~\ref{lemma:approximation_error_spectral_bound}, Lemma \ref{lemma:max_error_cos},
...
% Remember the definition of leverage scores, which play an important role in randomized linear algebra \parencite{DrMaMu06}.
...
% In particular, $\Det^{2} \bm{W} = \Det \bm{G} =  \prod\limits_{i \in [N]} \cos^{2} \theta_{i}(\mathcal{P}_{1},\mathcal{P}_{2})$.
\end{proposition}
...
%
%%%%
% Now, let $\bm{c}_{i}$ the columns of the matrix $\bm{C}(\bm{x})^{\Tran}$. 
...
\end{equation}
%= \bm{c}_{i}^{\Tran} \bm{K}(\bm{x}) \bm{c}_{i'}
Therefore
\begin{equation}
...
\end{equation}
% Now, recall that the $(e_{n}^{\mathcal{F}})_{n \in [N]}$ is an orthonormal family.
Moreover
\begin{equation}
...
% This boils down to showing that $\bm{K}(\bm{x})^{-1}$ is the matrix of the inner product $\langle \cdot,\cdot \rangle_{\cal F}$ restricted to ${\mathcal{T}(\bm{x})}$.
% Lemma
...
\end{align}
Now, $\displaystyle \sum\limits_{n \in \mathbb{N}^{*}} \sigma_{n} < \infty$ implies that $\displaystyle \sum\limits_{T \subset \mathbb{N}^{*}, |T| = N} \prod\limits_{t \in T} \sigma_{t} < \infty$. In fact, for $\ell \in [N]$ let $p_{\ell}$ the $\ell$-th symmetric polynomial. By Maclaurin’s inequality \parencite{Ste04}, and for any vector $\bm{\nu} \in \mathbb{R}_{+}^{M}$
\begin{equation}
...
% Measuring the error in RKHS norm has a computational advantage. Indeed, minimizing \eqref{CVS_e:residual} boils down to minimizing a quadratic form; and given a design $\bm{x}$ such that $\Det \bm{K}(\bm{x}) = \Det (k(x_i,x_j)) >0$, Equation~\eqref{CVS_e:residual} has a unique set of minimizing weights. The minimizer corresponds to the weights $\hat{\bm{w}} = \bm{K}(\bm{x})^{-1} \mu(\bm{x})$, where $\mu(\bm{x})\in\mathbb{R}^N$ contains the evaluation of $\mu$ at the $N$ design nodes $x_{i}$. Whenever the weights are chosen to be $\hat{\bm{w}}$, the sum in \eqref{CVS_e:residual} takes the same values as $\mu$ at the nodes $x_{i}$, and the optimal value of \eqref{CVS_e:residual} is thus called \emph{interpolation error}; otherwise we speak of \emph{approximation error}.
...
% The projection DPP that we have proposed in  This may make 

% Previous kernel-based randomized designs, i.e. continuous ridge leverage scores both i.i.d. \parencite{Bac17} and repulsive \parencite{BeBaCh19}, can be hard to implement in practice since they require  
...
% Volume sampling was originally introduced on a finite domain \parencite{DRVW06}, where it has been used in matrix subsampling for linear regression and low-rank approximations \parencite{DeWa17,BeBaCh18}. Like \parencite{BeBaCh19}, our work connects the discrete problem of subsampling columns from a matrix and the continuous problem of interpolating functions in an RKHS.
% }
...
% assume that $\mathcal{X}$ is equipped with a Borel measure $\mathrm{d}\omega$, and that the support of $\mathrm{d}\omega$ is $\mathcal{X}$.
...
% Denote by $\mathcal{F}$ the associated reproducing kernel Hilbert space (RKHS) of real-valued functions \parencite{BeTh11}.
...
% By construction, $\bm{\Sigma}$ is self-adjoint, positive semi-definite, and trace-class \parencite{Sim05}.
% Useful recall
...
%
% For $m \in \Ns$, with $\mathbb{N}^{*} = \mathbb{N} \smallsetminus \{0\}$, denote by $e_{m}$ the $m$-th eigenfunction of $\bm{\Sigma}$, normalized so that $\|e_{m}\|_{\mathrm{d}\omega} = 1$, and $\sigma_{m}$ the corresponding eigenvalue. Assumption~\ref{hyp:integrable_diagonal} implies that the embedding operator $I_{\mathcal{F}}: \mathcal{F} \longrightarrow \mathbb{L}_{2}(\mathrm{d}\omega)$ is compact; moreover, since $\mathrm{d}\omega$ is of full support in $\mathcal{X}$, $I_{\mathcal{F}}$ is injective \parencite{StCh08}. This implies a Mercer-type decomposition of $k$,
% \begin{equation}
% \label{CVS_eq:Mercer_decomp}
% 	k(x,y)= \sum\limits_{m \in \mathbb{N}^{*}}\sigma_{m}e_{m}(x)e_{m}(y),
% \end{equation}
% where the convergence is pointwise \parencite{StSc12}. The eigenvalues $(\sigma_m)$ are assumed to be non-increasing. Moreover, for $m \in \mathbb{N}^{*}$, we write $e_{m}^{\mathcal{F}} = \sqrt{\sigma_{m}}e_{m}$. Since $I_{\mathcal{F}}$ is injective, $(e_{m}^{\mathcal{F}})_{m \in \mathbb{N}^{*}}$ is an orthonormal basis of $\mathcal{F}$ \parencite{StSc12}. Unless explicitly stated, we assume that $\mathcal{F}$ is dense in $\mathbb{L}_{2}(\mathrm{d}\omega)$, so that $(e_{m})_{m \in \mathbb{N}^{*}}$ is an orthonormal basis of $\mathbb{L}_{2}(\mathrm{d}\omega)$. For more intuition, under these assumptions, $f \in \mathcal{F}$ if and only if $\sum_m \sigma_{m}^{-1} \langle f,e_{m} \rangle_{\mathbb{L}_{2}(\mathrm{d} \omega)}^{2}$ converges; 
...
% $
% is the so-called \emph{embedding}\footnote{When $g =1$ is constant, $\mu_{g}$ is classically called the mean-element of the measure $\mathrm{d} \omega$ \parencite{SmGrSoSc07}.} of $g$ in the RKHS $\F$.
...
% In the quasi-Monte Carlo (QMC) literature, several designs were investigated for $\X = [0,1]^{d}$, $g = 1$ and $\F$ that may not even be a Hilbert space; see \parencite{DiPi10}. In this context, the term \emph{QMC quadrature rule} means a low discrepancy sequence, loosely speaking a ``well-spread" set of nodes, along with uniform weights $w_{i} = 1/N$. If $\F$ is a Korobov space of order $s \geq 1$, the Halton sequence of nodes \parencite{Hal64} leads to $\mathcal{E}(\mu_1; \bm{x}, (1/N))^2$ in $\mathcal{O}(\log(N)^{2d} N^{-2})$ and higher-order digital nets converge faster as $\mathcal{O}(\log(N)^{2sd} N^{-2s})$ \parencite{DiPi14}[Theorem 5].
...
% %, and also simplices \parencite{Bas15,BaOw15}
...
% For instance, in the context of sampling problems in signal processing \parencite{Uns00,AlGr01,Avr18}.
...
% <<<<<<< HEAD
% This condition is more restrictive than what is required to belong to $\F$: $\|\mu_{g}\|_{\F}< +\infty$. Meanwhile, kernel interpolation only assumes that $\mu \in \F$. In this setting, several quantitative bounds were derived in the litterature, see \parencite{ScWe06} and \parencite{Wend04} for a modern review. In few words, these interpolation bounds typically depend on the \emph{fill-in distance}\footnote{In this setting, $\X$ is typically assumed to be a subset of $\mathbb{R}^{d}$.} $\varphi(\bm{x}) = \sup\limits_{y \in \X} \min\limits_{i \in [N]}\|y-x_{i}\|_{2}$, and the \emph{separation radius} $\rho(\bm{x}) = \frac{1}{2}\min\limits_{i\neq j}\|x_{i}-x_{j}\|_{2}$
...
% For example, \emph{kernel herding} \parencite{ChWeSm10} corresponds to uniform weights and greedily minimizing the LHS in \eqref{CVS_eq:wce_definition}.
...
This algorithm leads to an interpolation error that goes to zero with $N$ for a kernel of class $\mathcal{C}^{2}$ \parencite{DeScWe05}. Later, \cite{SaHa17} proved better convergence rates for smoother kernels. Again, these results assume that the domain $\X$ is compact.
% The theoretical analysis of this algorithm was carried out in \parencite{DeScWe05}  and . \rb{Can you say in what sentence what they prove for interpolation or quadrature? A uniform convergence rate?}
% Other greedy algorithms applied in this context includes \emph{kernel herding} \parencite{ChWeSm10} and in general the Frank-Wolfe algorithm (FW) \parencite{BaLaOb12}: these algorithms generate nodes without
Other greedy algorithms were proposed in the context of Bayesian quadrature (BQ) such as Sequential BQ \parencite{HuDu12}, or Frank-Wolfe BQ \parencite{BrOaGiOs15}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In general, each step of these greedy algorithms requires to solve a non-convex problem with many local minima \parencite{Oett17}[Chapter 5]. In practice, costly approximations must be employed such as local search in a random grid \parencite{LaLiBa15}.% or reduction to local convex problems \parencite{HiOe16}.
...
% Recall that in the context of kernel quadrature, obtaining convergence guarantees for the integration problem boils down to an interpolation problem for the embedding $\mu_{g}$. The literature is rich in results on the theoretical guarantees of designs adapted to specific domains $\X$ and $g \in \Ltwo$. For instance, the uniform grid for $\X = [0,1]$ was proved to be optimal for the approximation of the embedding of $g = 1$ in \parencite{Boj81} in the periodic Sobolev spaces of order $s$ with an optimal rate that corresponds to $\mathcal{O}(N^{-s})$. It is also asymptotically optimal with the same rate $\mathcal{O}(N^{-s})$ if the weight $g$ is not constant \parencite{NoUlWo15} but it is no longer optimal for small values of $N$ \parencite{ZhNo19}.

% In the quasi-Monte Carlo community, several sequences were proposed and analysed for integration problems in $\X = [0,1]^{d}$ and for $g = 1$ \parencite{DiPi14}. More precisely, the results deal with upper bounding the worst case \eqref{CVS_eq:wce_definition} integration error with uniform weights. For example, the Halton sequence \parencite{Hal64} have the rate of convergence in $\mathcal{O}(\log(N)^{2d} N^{-2})$.  Higher-order digital nets have convergence rates that scale as $\mathcal{O}(\log(N)^{2s(d-1)} N^{-2s})$ \parencite{DiPi10}.


% % The Fibonacci lattice have no equivalent in higher dimension $d \geq 3$.


% The convergence rates of quadratures based on QMC sequences and optimal weights (that minimizes the l.h.s of \eqref{CVS_eq:wce_definition}) inherits the rates of QMC sequences with uniform weights. For example, in \parencite{BOGOS2019}, the authors used this remark to prove rates of convergence of the quadrature based on high order digital nets and the corresponding optimal weights. This method can not apply for QMC sequences with lower order such as Halton sequence although numerical simulations suggests that they have the empirical convergence rate $\mathcal{O}(\log(N)^{2s(d-1)} N^{-2s})$ \parencite{Oett17}.
...
An alternative approach, that is very related to our contribution and has raised a lot of recent interest, is to observe that the squared power function \eqref{CVS_e:power} can be upper bounded by the inverse of $\Det \bm{K}(\bm{x})$ \parencite{Sch05,Tan19}.
% :
% \begin{equation}
% p(x;\bm{x})^{2} \leq \frac{\max\limits_{\tilde{\bm{y}} \in \X^{N+1}} \Det \bm{K}(\tilde{\bm{y}})}  {\Det \bm{K}(\bm{x})}.
% \end{equation}
 Designs that maximize $\Det \bm{K}(\bm{x})$ are called \emph{Fekete points}; see e.g. \parencite{BoMa02,BoDe11}.
...
\cite{KaSaTa19} proved that for the uni-dimensional Gaussian kernel, the approximate objective function of \parencite{Tan19} is actually convex. Moreover, \cite{KaSaTa19} analyze their interpolation error; see also Section~\ref{CVS_sec:main_theorems_2}.
...
% An alternative approach was proposed by , who  \cite{Tan19} thus proposes to build designs that maximize $\Det \bm{K}(\bm{x})$. , and the author  However,  \parencite{KaSaTa19}.
...
%
% Again, we emphasize that the analyses in \parencite{DeScWe05}, \parencite{SaHa17} and \parencite{KaSaTa19} assume that $\X$ is compact. \rb{and that $g$ is fixed?}
...
% % Following \parencite{Avr18}, we call these designs \emph{universal} \rb{not sure we should pretend to universality, maybe uniform is good enough.}.
...
% % Using an analysis similar to \parencite{Bac13} and \parencite{AlMa15} for the Nyström approximation and an extension of Matrix Bernstein concentration inequality for random operators \parencite{Min17}, the author proved that using nodes that are sampled according to the continuous ridge leverage score function
% \begin{equation}
...
% % Since the analysis of \cite{BeBaCh19} was inspired by previous work \parencite{BeBaCh18} comparing discrete projection DPPs with another discrete repulsive distribution called \emph{volume sampling} \parencite{DRVW06} for a column subset selection task. We follow the same steps of \parencite{BeBaCh18}, yet in the continuous domain, and we give the definitions of the continuous volume sampling with projection DPP in Section~\ref{CVS_sec:VS_DPP}.
% % Note that the necessary number of samples $N$ diverges when $\lambda \rightarrow 0$ since $d_{\lambda} \rightarrow \infty$.
% % Conversely, for a given number of nodes $N$, the approximation error can be bounded for typical RKHSs \parencite{Bac17}.
...
% Now, $\mathcal{S}$ is the image of the unit ball of $\Ltwo$ by the compact operator $\hat{\bm{\Sigma}}: \Ltwo \rightarrow \F$.\rb{Did we define this new oeprator somewhere?} The adjoint of this operator is $\hat{\bm{\Sigma}}^{*} = \mathcal{I}_{\F}$ \parencite{StSc12}, so that $\bm{\Sigma} = \hat{\bm{\Sigma}} \mathcal{I}_{\F} = \hat{\bm{\Sigma}} \hat{\bm{\Sigma}}^{*}$.
% Finally, by the following result (Theorem 2.2; Chapter 4; \parencite{Pin12}) \footnote{The result is stated in terms of the $s$-numbers of $\hat{\bm{\Sigma}}$ that are equal to the square root of the respective eigenvalues of $\bm{\Sigma}$ because $\bm{\Sigma} = \hat{\bm{\Sigma}} \hat{\bm{\Sigma}}^{*}$.}:
...
% Definition~\ref{CVS_def:VS} could be mistaken with the definition of a determinantal point process (DPP; \citealp{Mac75}). However, the cardinality of a DPP sample is a sum of Bernoulli random variables \parencite{HoKrPeVi06}, while volume sampling is supported on subsets of $\X$ with cardinality exactly equal to $N$. This property is convenient for approximation tasks where the number of nodes $N$ is fixed. While it is not a DPP, 
...
% One can show that the cardinal of a random subset that follow the distribution of the projection DPP is a.s. equal to $N$ \parencite{HoKrPeVi06}.
...
%%%%%%%%%%%%%Removed sentences%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% In practice, sampling each conditional, using e.g. rejection sampling \cite{RoCa04}, requires the knowledge of proposals that upper bound the $N$-leverage score function: $x \mapsto \KDPP(x,x)$. Sampling from such distribution become harder for large values of $N$ as it was mentioned in \parencite{BaHa16}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% The evaluation of the projection kernel $\KDPP$ defined in Chapter~\ref{chap:dppkq} requires the knowledge of the spectral decomposition of $\bm{\Sigma}$. The algorithm of \cite{HoKrPeVi06} implements the chain rule for projection DPPs, and each conditional is sampled using rejection sampling. 
...
where $p$ is the power function defined in \eqref{CVS_e:power}. Algorithm~\ref{alg:approx_HKPV_SAMPLER} proposed in \parencite{ReGh19} uses this decomposition to approximate continuous volume sampling. 
% Therefore, by successively sampling proportionally to the power function 
 The output of the algorithm follows a density  $\tilde{f}_{\VS}$ with respect to $\prod_{n \in [N]} \mathrm{d}\omega(x_{n})$ that satisfies \parencite{ReGh19}[Lemma 4.4]
\begin{equation}\label{CVS_eq:approximate_CVS}
\forall (x_{1}, \dots, x_{N}) \in \X^{N}, \:\: \tilde{f}_{\VS}(x_{1}, \dots,x_{N}) \leq N!^{2} f_{\VS}(x_{1}, \dots,x_{N}).
\end{equation} 
This bound is irrelevant for large values of $N$. In other words, the approximation of continuous volume sampling by the Algorithm~\ref{alg:approx_HKPV_SAMPLER} is very loose. Note however that $\tilde{f}_{\VS} = f_{\VS}$ if $k$ is a projection kernel that defines an integration operator of rank $N$: Algorithm~\ref{alg:approx_HKPV_SAMPLER} coincides with Algorithm~\ref{alg:HKPV_SAMPLER} in this case.


\begin{figure}[!ht]
 \centerline{
 \scalebox{0.9}{
\begin{algorithm}{$\Algo{A-HKPV}\big(k, \mathrm{d}\omega \big)$}
\Aitem $n \longleftarrow  1$
\Aitem $\mu(x) \propto  p(x; \emptyset) \mathrm{d}\omega(x)$ \ar{Initialize the distribution}
\Aitem $\bm{x} \longleftarrow  \emptyset$
\Aitem \While $n \leq N $
\Aitem \mt  Pick $x_{n}$ in $\mathcal{X}$ from $\mu$ \ar{Sample from the distribution}
\Aitem \mt  $\bm{x} \longleftarrow  \bm{x} \cup \{x_{n}\}$
\Aitem \mt $n \longleftarrow  n+1$
\Aitem \mt $\mu(x) \propto p(x;\bm{x}) \mathrm{d}\omega(x)$ \ar{Update the distribution}
\Aitem \Return $\bm{x}$
\end{algorithm}
 }
 }
\caption{Pseudocode of approximate continuous volume sampling based on a variant of the HKPV.}
\label{alg:approx_HKPV_SAMPLER}
\end{figure}
...
A workaround would be to use $\tilde{f}_{\VS}$ as the density of the initial state of an MCMC sampler proposed in \parencite{ReGh19}.
\rev{
This MCMC algorithm is based on a Gibbs sampler chain: given a state $\bm{x} = \{x_{1}, \dots , x_{N} \}$, remove a node $x_{n}$ chosen uniformly at random and add a node $y$ with a probability proportional to $\Det \bm{K}(\bm{x}')$ where $\bm{x}' = \{x_{1}, \dots, x_{n-1}, y,x_{n+1}, \dots, x_{N} \}$.}
%, and the initial state of the Markov chain follow a distribution of density $p_{0}$ defined on $\mathcal{X}^{N}$ with respect to $\prod\limits_{n \in [N]} \mathrm{d}\omega(x_{n})$. 
...
% \footnote{This term appears in \parencite{NoUlWo15} for the description of the initial error for quadrature rules adapted to oscillatory weights in the case of the periodic Sobolev space.} .
...
\begin{figure}
    \centering
    \resizebox{0.6\textwidth}{!}{\input{img/unit_balls/func_unit_balls_3.tex}}
...
then
\begin{equation}
\mathcal{B}_{N}(f,g) = \sum\limits_{n \in \Ns} \langle f,e_{n} \rangle_{\mathrm{d}\omega} \langle g,e_{n} \rangle_{\mathrm{d}\omega}\left(1- \EX_{\VS}\tau_{n}^{\F}(\bm{x}) \right) .%\label{CVS_eq:VS_bias}
\end{equation}
Moreover,
...
% \begin{equation}
% \epsilon_{m}(N) = \EX_{\VS} \|\mu_{e_{m}} - \bm{\Pi}_{\mathcal{T}(\bm{x})} \mu_{e_{m}}\|_{\F}^{2} =  \sigma_{m} \EX_{\VS} \|e_{m}^{\F} - \bm{\Pi}_{\mathcal{T}(\bm{x})} e_{m}^{\F}\|_{\F}^{2}.
% \end{equation}
...
% \end{equation}
 % First, the approximation error $\displaystyle \|\mu_{g} - \bm{\Pi}_{\mathcal{T}(\bm{x})} \mu_{g}\|_{\F}^{2}$ can be decomposed in terms of quantities that quantify the square of the interpolation error of the embeddings $\mu_{e_{m}}$ where $m \in \mathbb{N}^{*}$.
...
\end{equation}
with $s \in \{1,2,3,4,5\}$.
% Consider the uni-dimensional periodic Sobolev space of smoothness parameter .
% The eigenvalues have a polynomial decay; see \parencite{Wah90}. We take for .
...
\end{equation}
 ...
 We use the Gibbs sampler proposed by \parencite{ReGh19} to approximate continuous volume sampling.
  ...
  % This corresponds roughly to the empirical mixing time observed in \parencite{ReGh19} that was showed to scale as $\mathcal{O}(N^{2})$ (compared to the theoretical scaling in $\mathcal{O}(N^{5})$).
...
% \begin{figure}[]
%     \centering
% \includegraphics[width= 0.45\textwidth]{img/icml/simulation/EX_VS_err_n_s_2_th_vs_emp_exp20_1}\\
...
% <<<<<<< HEAD
% =======
%  In this paper, we introduced and analyzed {\em continuous volume sampling} for kernel based interpolation. This work establishes a new connection between repulsive sampling in the discrete domain \parencite{DRVW06,DeWa17,BeBaCh18} and in the continuous domain \parencite{Bac17,BeBaCh19}.
% %
% This new distribution also depends on the RKHS kernel $k$.

%  Continuous volume sampling has many advantages over existing deterministic optimisation algorithms discussed in Section~\ref{CVS_sec:sequential_algos}. First, our theoretical analysis requires minimal assumptions: Assumption~\ref{CVS_hyp:integrable_diagonal} is satisfied by all the kernels used in practice, while Assumption~\ref{CVS_hyp:beta_N_bounded} .... This is to be compared to the theoretical toolbox of scattered data approximation \parencite{Wend04} that assumes that the domain $\X$ is compact and the design is not proposed in general. Second, the implementation of these deterministic algorithms require approximations that can make the ...

%  It would be possible to extend the derandomization of discrete volume sampling proposed in \parencite{DeRa10}. This extension would give a new determinisitc algorithm with guarantees matching the lower bounds.

%  Our analysis imply sharp bounds, matching the lower bounds, avoiding non optimal upper bounds on the Lebesgue constant as in the analysis of approximate Fekete points \parencite{KaSaTa19}. Yet, it will be interesting to analyse to prove sharp bounds for the Lebesgue constant under continuous volume sampling.
...
\end{equation}
see e.g. Lemma 21 in \parencite{HoKrPeVi06}. Finally, the monotone convergence theorem allows us to conclude
\begin{equation}
...
% The eigenvalues have a polynomial decay; see \parencite{Wah90}. We take for $m \in \Ns$, $\sigma_{m}=m^{-2s}$ \footnote{We drop the potential multiplicities of the eigenvalues by simplicity.}.
...
% \end{equation}
% for an $M \geq N$ sufficiently large. The numerator and denominator of the right hand side of \eqref{CVS_eq:error_numerical_approximation} can be calculated using an efficient algorithm for the calculation of the elementary symmetric polynomials \parencite{KuTa12}[Algorithm 7].
...
% Consider now the RKHS generated by Gaussian kernel and the Gaussian measure. We take for all $m \in \Ns$, $\sigma_{m}=\alpha^{N}$  \parencite{ZhWiRoMo97}, for some $\alpha \in [0,1[$.
...
% We started this manuscript by a review on the recent work on subsampling problems for problems with some linear structure. 

% This thesis has developed studied some subsampling problems with linear structure through the lens of determinantal point process. 
...
% Still, lengths contain only the , and one may expect to improve the quality of sampling by considering 
...
% We tried to give an overview of the existing literature for every approximation task treated in this manuscript to shed the light on the existing "concurrent" algorithms. 
% The purpose behind the lengthy bibliographic work conducted in this thesis was to emphasize on the richness of the literature for the three approximation tasks that we have studied. 
...
It seems that the impact of DPPs and their variants may be limited for sub-sampling in discrete sets. Indeed, as it was mentioned in the introduction, DPPs and their variants replace leverage score sampling in the minimalist sampling regime, i.e. when the sub-sampling budget is equal to the dimensionality of the problem. This can be useful for applications such as feature selection where we need to keep the smallest possible number of columns. However, for other sub-sampling problems, this stringency is not necessary and DPPs may easily be replaced by leverage scores sampling. In other words, in discrete sets, sub-sampling using first-order information is sufficient for the majority of problems and there is no need to use high-order information!
 % expressed by volumes don't allows to improve the 
% Indeed, the literature is already abundant with solutions that may be way more simple to implement. 
% leverage scores sampling may have better  empirical performance compared to dual volume sampling in Bernstein regime of sampling. In other words, 
...
% ....
we have two different levels of impact that corresponds to two different situations. In the first situation, the RKHS is "classical" and there exists some configuration with good approximation guarantees for kernel quadrature or kernel interpolation. This is the case of 
the periodic Sobolev space where the uniform grid is known to achieve the optimal rates. In these cases, we know from the theoretical and the empirical results that the projection DPP and continuous volume sampling would be as competitive as the deterministic sequence but with the supplementary cost of sampling.


In the second situation, the RKHS is not classical and there is no simpler alternative algorithm that can  compete with the optimality of the projection DPP or continuous volume sampling. In these cases DPPs and their variants cannot be replaced! 
...
% % highlighted that the impact of DPPs for this 


% a hard to be replaced algorithm.


% the use of continuous volume sampling distribution for the task of kernel quadrature and for the slightly more general task of kernel interpolation.


% % the slightly more general problem of kernel interpolation.

% % the task is to s

% % Formally, 

% % such that the set of representative set of nodes in $\X$ for the numerical integration task

%  % Not only This new parametrization reinforces the geometric intuition of projection DPPs  that seem to be useful in the development of 

% In particular, we challenged the optimality of volume sampling for this task by looking for another alternative whithin all the possible options offered


% and the potential improvement of the existing results.


% Indeed, it is immediate to see that volume sampling is a mixture of projection DPPs and 

% We started this manuscript by questioning  This question was the motive to adopt a new interpretation of determinantal point processes: they define naturally random subspaces that tend to be close to some reference subspace. This new interpretation 
...
The empirical investigations in Chapter~\ref{chap:dppkq} and Chapter~\ref{chap:CVS} were restricted to  classical RKHSs such as Sobolev spaces and Gaussian spaces. Nevertheless, our theoretical results are applicable to a broader class of less-known kernels.


In particular, our results are valid for RKHSs defined by \emph{localization operators}. For example, the RKHS  defined by band-limited functions restricted to a compact interval $I$ of $\mathbb{R}$, generated by the Sinc kernel and the uniform measure on $I$ was the topic of intense research in signal processing \parencite{SlPo61,LaPo61}. This RKHS contains band-limited functions truncated in the interval $I$, and represent band-limited signals measured in a limited span of time, which is a more realistic class of functions to work with in signal processing.

 % that the classical interest in this  stems from its realism: it contains band-limited signals . 

 Our approach applies to this RKHS, since the ...

 % Working in this RKHS is more  than the RKHS of band-limited functions 
 ...
 % of the sinc kernel. In particular, the eigenvalues and eigenfunctions of  the corresponding integration operator was the topic of many research in functional analysis.   
...
Manifold landmarking \citep*{GaKoDa19}\citep*{EhGrCh19} would be another field of application for continuous volume sampling. Indeed, for these domains, the spectral decomposition of  the integration operator is usually intractable.
 ...
 % for which approximate sampling is possible without the spectral decomposition

% , that were conducted in , showed that optimal kernel quadrature on projection DPP nodes is a reliable quadrature with strong empirical behaviour and plausible theoretical guarantees.  
% This suggest to extend this empirical investigation to other less-known kernels. Indeed, 

% We emphasize that the purpose of these experiments was the numerical validation of the . In particular, practical applications of work includes interpolation tasks in signal processing and numerical integration tasks in finance.

% As for signal processing, many kernels satisfy the required conditions: 


% founds i canA perspective of future work will be to implement the proposed algorithms in other RKHSs that are recurrently used in signal processing or in finance. 


 % purpose of this work is to give a universal treatment of kernel quadrature and kernel interpolation under DPPs and their variants.


% of any universal theory is to extend t
% The work done in Chapter~\ref{chap:dppkq} and Chapter~\ref{chap:CVS} motivates the exploration 
...
% \end{equation} 
% In particular, this assumption guarantees that $\sum_{m \in \mathbb{N}^{*}} \sigma_m <+\infty$. This hypothesis is important in the proofs of \parencite{Bac17} too. 
...
% Note however, that the previous work on DPPs is  functional subspaces do not correspond to some RKHS, because a Sobolev space of order $s$ This functional space can be represented by an RKHS if and only if $s>d/2$ \parencite{BeTh11}[Theorem 132].


%  For instance, in Theorem??, the orthonormal basis is $(e^{2\pi \mathbf{i} m})_{m \in \mathbb{Z}}$ and for every $m \in \mathbb{Z}^{*}$ $\sigma_{m} = 1/m$, yet the rate of convergence scales as $\mathcal{O}(N^{-1}\sigma_{N})$.


% as we have seen in Theorem~\ref{thm:dirichlet_quadrature}, $\mathbb{L}_{2}(\mathrm{d}\omega)$ was chosen to be the Fourier family, and ...
% \begin{equation}
% \|f\|^{2}_{\F} = \sum\limits_{m \in \mathbb{Z}^{d}} (1+ \|m\|_{\infty})^{2s} |\hat{f}_{m}|^{2}.
% \end{equation}

% However, as we have seen in Theorem~\ref{thm:dirichlet_quadrature}, for $s\leq d/2$, the asymptotic rate of convergence is $\mathcal{O}(N^{-1-2s/d})$ which is faster than the rate $\mathcal{O}(\sigma_{N+1})$ that is $\mathcal{O}(N^{-2s/d})$. This observation goes for the two other quadratures  \parencite{BaHa16,Joh97}.
%  and 

%  In these definition of $\F$ assume from 
% \begin{equation}
% ..
% \end{equation}


% For example, in the case of the periodic Sobolev space of order $s$. This functional space can be represented by an RKHS if and only if $s>d/2$ \parencite{BeTh11}[Theorem 132]. Intuitively, for $s \leq d/2$, the norm of $\F$ is too "coarse" to make the evaluation functions $x \mapsto f(x)$ continuous. Now, as we have seen in Theorem~\ref{thm:dirichlet_quadrature}, for $s\leq d/2$, the asymptotic rate of convergence is $\mathcal{O}(N^{-1-2s/d})$ which is faster than the rate $\mathcal{O}(\sigma_{N+1})$ that is $\mathcal{O}(N^{-2s/d})$. This observation goes for the two other quadratures  \parencite{BaHa16,Joh97}.

%  for $s=1/2$, using , for a trigonometric function $g$
% \begin{equation}
%   \frac{1}{N}\sum_{n \in [N]} g(e^{\mathbf{i}\theta_{n}}) 
%   \xrightarrow[N \rightarrow +\infty]{\text{law}} \mathcal{N}(0,\frac{2}{N}\sum_{m \in \mathbb{N}^{*}} m|\hat{g}_{m}|^{2}).
%  \end{equation}
...
% We illustrate this point by the case of the case of periodic Sobolev space of some order 
 ...
 % there still some grey areas on this topic that we believe should be lightened. In particular, we point a an intriguing remark on the rates of convergence of DPP based quadratures. 
...
% the analysis of column subset selection and kernel quadrature under the distribution of a  projection DPP makes use of the control of symmetric functions on the principal angles b
% In this thesis, we have introduced  technical contribution of this thesis is to use principal angles to control 
...
% the expected value of symmetric polynomials of the inver
...
% Nevertheless, some recent advances in matrix sparsification that relies on the analysis of the expected characteristic polynomial \citep*{MaSpSr15} 

% Tools from the work  is at least the case regarding the work of techniques developed in  for the resolution of the long standing Kadison-Singer conjecture. 


% The expected characteristic polynomial was already studied for the study of the  problem~...  

% Deriving bounds of $\EX_{\DPP} \frac{1}{{}\cos^{2} \theta_{N}(\mathcal{T}(\bm{x}),\mathcal{E}^{\mathcal{F}}_{N})}$ 



% Moreover, as we have seen in Section??, the empirical rates of optimal kernel quadrature under the distribution projection DPP
...
% This new perspective leads to unify the treatement of discrete 


% sharp theoretical  unified theoretical analysis ...
