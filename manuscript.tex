...
\caption{Squared interpolation error vs. number of nodes $N$ for $s = 1$ (left) and $s=3$ (right).
...
We consider $g$ to be an eigenfunction $\bm{\Sigma}$: $g \in \{e_{5},e_{10},e_{14},e_{15},e_{16},e_{17}\}$, so that $\mu_{g} = \bm{\Sigma}g \in \{\sigma_{5}e_{5},\sigma_{10}e_{10},\sigma_{14}e_{14},\sigma_{15}e_{15},\sigma_{16}e_{16},\sigma_{17}e_{17}\}$. We observe the interpolation error under the projection DPP (DPPKQ) for $N \in [5,50]$. Figures~\ref{fig:pDPP_results_pSobolev_exp_2} and ?? show log-log plots of the interpolation error  w.r.t. $N$, averaged over 50 samples for each point, for $s \in \{1,3\}$.
...
\end{equation}
where $\mathcal{I} = \{ \frac{1}{\sqrt{\Delta}} (\epsilon_{1}, \dots, \epsilon_{\Delta}), \epsilon_{i} \in \{-1,1\} \}$ for some $\Delta \in \mathbb{N}^{*}$. The quantity~\eqref{eq:surrogate_worst_inerpolation_error} is an approximation of the worst interpolation error. Indeed, if $g = \sum\limits_{\delta \in [\Delta]} \epsilon_{\delta} e_{\delta}$ for some $(\epsilon_{1}, \dots, \epsilon_{\Delta})$, then $\|g\|_{\mathrm{d}\omega} = 1$ and the quality of the approximation improves as $\Delta \rightarrow +\infty$.

Again, we compare the following quadratures defined previously: $(i)$ the quadrature rule DPPKQ, $(ii)$ LVSQ with regularization parameter $\lambda = 0$, and $(iii)$ UGKQ. We take $N \in [5,50]$. Figures~\ref{fig:pDPP_results_pSobolev_exp_3} and ?? show log-log plots of the worst case quadrature error  w.r.t. $N$, averaged over 50 samples for each point, for $s \in \{1,3\}$.
...
\caption{Squared interpolation error vs. number of nodes $N$ for $s = 1$ (left) and $s=3$ (right).
\label{fig:pDPP_results_pSobolev_exp_3}}
\end{figure}
...
We observe that the worst interpolation errors, of the three quadratures, converge  to $0$. DPPKQ converges at the rate $\mathcal{O}(N^{-2s})$ better than the theoretical rate $\mathcal{O}(N^{3-2s})$ of Theorem~\ref{thm:main_theorem_improved}. UGKQ seems to have a better rate than $\mathcal{O}(N^{-2s})$ for $s=3$. ?????
...
Remember that the Mercer decomposition of $k_{s,d}$ with respect to the measure $\mathrm{d}\bm{\omega}(x) = \prod\limits_{\delta \in [d]} \mathrm{d}\omega(x_{\delta})$ writes

\begin{equation}
k_{s,d}(x,y) = \sum\limits_{u \in (\mathbb{N}\smallsetminus \{0\})^{d}} \prod\limits_{\delta \in [d]} \sigma_{\delta} e_{\delta}(x_{\delta}) e_{\delta}(y_{\delta}),
\end{equation}

where the $(\sigma_{m})$ and the $(e_{m})$ are respectively the eigenvalues and the eigenfunctions of the kernel $k_{s}$ with respect to the measure $\mathrm{d}\omega$.
...
% as in the univariate case where $\sigma_{1} \geq \sigma_{2}\geq\dots$.
 ...
 Whenever the univariate eigenvalues take the form $\sigma_{i} = \frac{1}{(1+i)^{\eta}}$ with $\eta >0$, such as in the Korobov case, it holds
\begin{align}
...
We define $u \prec v$ to be equivalent to \eqref{e:orderSobolev}; and we define $\textbf{u}= (u^{1}, u^{2},\dots)\in (\mathbb{N} \smallsetminus \{0\})^d$ the family of multi-indices ordered according to $\prec$. The repulsion kernel is defined as
\begin{equation}
\KDPP(x,y) = \sum\limits_{n \in [N]} \prod\limits_{\delta \in [d]}e_{u^{n}_{\delta}} (x_{\delta}) e_{u^{n}_{\delta}} (y_{\delta}), \quad x,y\in [0,1]^d.
\end{equation}
