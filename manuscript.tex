...
We supply $M(\Omega)$ with the $\sigma$-algebra $\mathcal{M}(\Omega)$ generated by the evaluation maps
\begin{equation}
ev_{B}: M(\Omega) \rightarrow \mathbb{R}\cup \{\infty \}.
\end{equation}

In other words, $\mathcal{M}(\Omega)$ is the smallest $\sigma$-algebra for which all the evaluations maps $ev_{B}$, $B \in ??$, are measurable.

An example of measurable subset of $M(\Omega)$ is the set of counting measures denoted $N(\Omega)$: an element $\mu$ of $N(\Omega)$ takes values on $\mathbb{N}$
\begin{equation}
\mu(B) \in \mathbb{N}.
\end{equation}
...
\begin{proposition}
We have $N(\Omega) \in \mathcal{M}(\Omega)$ and the trace $\sigma$-algebra is generated by the system
\begin{equation}
....
\end{equation}
\end{proposition}
...
\mu(B) \in \{0,1\}.
\end{equation}
The set of simple counting measures is denoted $N_{0}(\Omega)$.

Discrete subsets of $\Omega$ can be identified to simple counting measures by considering the corresponding atomic measures. Thereby, general counting measures can be seen as discrete subsets with multiplicities. In this manuscript, we will be mostly interested in simple counting measures; even tough we give ...

Now, we move to the definition of $\sigma$-algebras on the sets $M(\Omega)$, $M^{+}(\Omega)$, $N(\Omega)$ and finally $N_{0}(\Omega)$. 
...
\begin{definition}
A random measure $\gamma$ on $\Omega$ is a measurable map from some probability space into the measurable space $(\mathbb{M}^{+}(\Omega), \mathcal{M}^{+}(\Omega))$. The distribution of $\gamma$ is given by the probability measure $\mathbb{P}_{\gamma}$ defined on $\mathcal{M}^{+}(\Omega)$ by
\begin{equation}
\mathbb{P}_{\gamma}(B) = \mathbb{P}(\gamma \in B).
\end{equation}
...
\begin{example}
Consider $\mu$ a probability measure on $\Omega$ and let $\nu$ be an integer valued random variable. Let $x_{1}, x_{2} \dots, $ be independent random variables in $\Omega$ with distribution $\mu$.
Then the random measure $\gamma$ defined by
\begin{equation}
\gamma = \sum\limits_{n \in [\nu]} \delta_{x_{n}},
\end{equation}
is a point process on $\Omega$. This process is called the mixed binomial process with mixing distribution ?? and sampling distribution $\mu$.

If $\nu = N$ a.s., we recover the binomial process: for every $B \in \mathcal{B}$,
\begin{equation}
\forall n \in \{0, \dots, N\}, \:\: \mathbb{P}(\gamma(B) = n) = \binom{N}{n}\mu(B)^{n}(1-\mu(B))^{N-n}.
\end{equation}

If $\nu$ follows the distribution of a Poisson random variable of parameter $\lambda$, then $\gamma$ we recover a Poisson point process of intensity $\lambda \mu$ that we define in the following.
\begin{equation}
...
\item for every $B_{1}, \dots, B_{m} \in \mathcal{B}$ pairwise disjoint sets, the random variables $\gamma(B_{1}), \dots, \gamma(B_{m})$ are independent. 
\end{itemize}
\end{definition}

In other words, for a Poisson point process of intensity $\mu$, for every $B_{1}, \dots, B_{m} \in \mathcal{B}$ pairwise disjoint sets
...
An alternative description of the distribution of a point process is offered by the joint intensities. We give in  the definitions of these measures to simple point processes.

We start with the first intensity.  
\begin{definition}
...
\end{equation}
\end{definition}
The intensity measure is well defined but may take infinite values. In particular, it is well defined for point processes.


\begin{example}
Let $\mu$ be a $\sigma$-finite measure on $\Omega$ and let $\gamma$ be a Poisson point process of intensity $\mu$. It is immediate from~\eqref{eq:poisson_cylinder_prb} that 
\begin{equation}
\forall B \in \mathcal{B}, \:\: \mathbb{E}\gamma(B) = \mu(B).
\end{equation}
Therefore, the first intensity of $\gamma$ is simply $\mu$.
\end{example}



If $\gamma_{1}$ is absolutely continuous with respect to $\mathrm{d}\omega$ then the Radon Nikodym derivative $\rho_{1}$ is called the first intensity function and satisfies
\begin{equation}\label{eq:first_intensity_function}
\mathbb{E} \gamma(W) = \int_{W} \rho_{1}(x) \mathrm{d}\omega(x). 
\end{equation}
The intuition behind~\eqref{eq:first_intensity_function} depend on $\Omega$. We give two examples.

\begin{example}
Let $\Omega$ be a finite set and define the measure $\mathrm{d}\omega = \sum\limits_{x \in \Omega} \delta_x$. Then 
\begin{equation}
\rho_{1}(x) = \mathbb{P}( x \in \gamma ).
\end{equation}

\end{example}

\begin{example}
$\Omega = \mathbb{R}$ and $\mathrm{d}\omega$ is the Lebesgue measure then for every $x \in \mathbb{R}$, ... then
\begin{equation}
\rho_{1}(x) = ...
\end{equation}
\end{example}
...
\begin{definition}
Let $\gamma$ be a simple point process. The joint intensities of $\gamma$ with respect to $\mathrm{d}\omega$ are functions $\rho_{L}: \Omega^{L} \rightarrow [0,+\infty[$ for $\ell \in \mathbb{N}^{*}$, such that for any family of mutually disjoint Borel subsets $W_{1}, \dots, W_{L}$ of $\Omega$
\begin{equation}\label{eq:pointprocess_intensity}
\mathbb{E} \prod\limits_{\ell \in [L]} \gamma(W_{\ell}) = \int_{\prod\limits_{\ell \in [L]}W_{\ell}} \rho_{L}(x_{1}, \dots, x_{L}) \mathrm{d}\omega(x_{1}) \dots \mathrm{d}\omega(x_{L}).
\end{equation}
\end{definition}
