...
We continue on this direction on Chapter~\ref{chap:dppkq} and Chapter~\ref{chap:CVS} where we study a new class of quadratures and interpolations based on repulsive nodes that follow the distribution of a determinantal point process or one of its variants: continuous volume sampling. Before that, we review the existing work on volume sampling in the following section.
...
\end{equation}
where $\Pi_{k}$ is the projection on the first left eigenvectors of $\bm{X}$.
