...
Many tasks in applied mathematics can be reformulated as problems of sensing under budget constraints, that is the estimation of some function of the data based on a partial knowledge of the available data; this function can be a scalar, a vector, a matrix or even an element of a Hilbert space. Examples of these sensing problems include subset selection, numerical integration or interpolation. 
...
\begin{definition}
A random measure $\gamma$ on $\mathcal{D}$ is a measurable map from some probability space $(\Omega, \mathcal{A}, \mathbb{P})$  into the measurable space $(\mathbf{M}(\mathcal{D}), \mathcal{M}(\mathcal{D}))$. The distribution of $\mathcal{D}$ is given by the probability measure $\mathbb{P}_{\gamma}$ defined on $\mathcal{M}(\mathcal{D})$ by
\begin{equation}
...
Let $G$ be a finite graph and denote $\mathcal{D}$ the set of edges of $G$ and $\mathrm{d}\omega = \sum\limits_{e \in \mathcal{D}} \delta_{e}$ be the counting measure on $\mathcal{D}$. Now, let $\bm{K} \in \mathbb{R}^{|\mathcal{D}| \times |\mathcal{D}|}$ be the transfer-impedance matrix: for $e,e^{'} \in \mathcal{D}$, $\bm{K}_{e,e^{'}}$ is  ..... Let $T$ be a random spanning tree chosen uniformly from the
...
The spherical ensemble is another example of a DPP based on the eigenvalues of random matrices. Interestingly, this ensemble have two representations; it can be seen as a DPP on the complex plan or as a DPP on the  sphere $\mathbb{S}^{2}$, hence the name. This ensemble was already used in theoretical physics for the modeling of repulsive particles in the sphere [??] [??]. The connection with eigenvalues of random matrices was proved by ??? 



\begin{theorem}[Theorem 5.0.4 in ???]
Let $A,B$ be independent $N\times N$ matrix with i.i.d. standard complex Gaussian entries. Define the set of eigenvalues $\{z_{1}, \dots, z_{N}\} \subset \mathbb{C}$ of $A^{-1}B$; and for every $n \in [N]$ let $x_n$ be the stereographic projection of $z_n$ from the complex plan to the sphere $\mathbb{S}^{2}$.



Then, $\{z_{1}, \dots, z_{N}\}$ follows the distribution of the determinantal point
process associated to the kernel defined on the complex plan
\begin{equation}
\kappa_{\mathbb{C}}(z,w) = (1+z\overline{w})^{N-1},
\end{equation}
with respect to the reference measure $\mathrm{d}\omega(z) = \frac{N}{\pi(1+|z|^{2})^{N+1}} \mathrm{d}\lambda(z)$, where $\mathrm{d}\lambda$ is the Lebesgue measure on $\mathbb{C}$; and $\{x_{1}, \dots, x_{N}\}$ follows the distribution of the determinantal point
process associated to the kernel
\begin{equation}
\kappa_{\mathbb{S}^{2}}(x,y) = \sum\limits_{n=1}^{N} ...
\end{equation}
...
\subsubsection{The numerical simulation of a projection DPP}
...
\Prb \big( |\gamma| = N \big) = 1,
 ...
 where $N$ is the rank of the integral operator associated to the kernel $\KDPP$. For this reason, we can identify such a random set with a sample from a random variable defined on $\mathcal{X}^{N}$.
...
\subsubsection{Periodic Sobolev spaces and Korobov spaces}
...
Assume that $\mathcal{X}$ is a compact space and $\mathrm{d}\omega$ is a non-degenerate finite Borel measure on $\mathcal{X}$.
...
\end{theorem}
The existence of the orthonormal basis $(e_m)_{m \in \mathbb{N}^{*}}$ is a consequence of the spectral theorem applied to the compact operator $\bm{\Sigma}$ \footnote{The compactness of $\bm{\Sigma}$ is proved using the Arzelà–Ascoli theorem applied to the compact set $\X$. }. The continuity of the $e_{m}$ corresponding to non vanishing eigenvalues is a consequence of
\begin{equation}
e_{m}(.) = \frac{1}{\sigma_m} \int_{\X}k(.,y) e_{m}(y) \mathrm{d}\omega(.),
\end{equation}
and $e_{m} \in \F$. 
Moreover, as a consequence of Theorem~\ref{thm:Mercer_for_compact}, the family $(e_{m}^{\F} = \sqrt{\sigma_m}e_m; \sigma_m>0)_{m \in \mathbb{N}^{*}}$ is an o.n.b. of $\mathcal{F}$; see Theorem?? in \cite{CuZh07}. In particular
\begin{equation}
\|e_{m}^{\F}\|_{\F}^{2} = \sigma_{m}\|e_{m}\|_{\F}^{2} = 1,
\end{equation}
and the coefficients of an element $f \in \F$ in the o.n.b. $(e_{m}^{\F})_{m \in \mathbb{N}^{*}}$, depend??? on the coefficients of $f$, seen as an element of $\mathbb{L}_{2}(\mathrm{d}\omega)$, in the o.n.b. $(e_{m})_{m \in \mathbb{N}^{*}}$. Indeed
...
% Now, let $f \in \F$, and 
\begin{equation}
f  = \sum\limits_{m \in \mathbb{N}^{*}} \langle f, e_m \rangle_{\mathrm{d}\omega} e_m =  \sum\limits_{m \in \mathbb{N}^{*}} \frac{\langle f, e_m \rangle_{\mathrm{d}\omega}}{\sqrt{\sigma_m}} e_{m}^{\F},
\end{equation}
so that
\begin{equation}
...
The identity~\eqref{eq:rkhs_norm_ltwo_norm} proves that the operator $\bm{\Sigma}^{1/2}: \mathbb{L}_{2}(\mathrm{d}\omega) \rightarrow \F$ is an isometry, and $\mathcal{F}$ can be seen as an ellipsoid in $\mathbb{L}_{2}(\mathrm{d}\omega)$ as it is illustrated in Figure~\ref{fig:rkhs_is_ellipsoid}: the principal axis correspond to the directions spanned by the eigenfunctions $e_{m}$.
