...
As we have seen in the introduction, the Newton-Cotes quadrature is based on equi-distanced nodes on $[a,b]$ and it is exact for polynomials of order smaller than $N-1$ when $N$ nodes were used. Gauss observed that this configuration (the equi-distanced nodes) is not optimal and shed the light on how to design the configuration $x_{1}, \dots, x_{N}$ in a quadrature. This observation gave birth to the field of Gaussian quadratures.
...
This characterization of the Gauss nodes highlighted the importance of the emerging notion of orthogonality between polynomials. Thanks to the work of Mehler [??], Christoffel [??] and others, this notion of orthogonality was pushed further to encompass quadrature rules with respect to  weighted measures: $\mathrm{d}\omega = w(t) \mathrm{d} t$ 
...
\begin{equation}\label{eq:OP_reccurence}
t P_{m}(t) = a_{m}P_{m+1}(t) + b_{m}P_{m}(t) + a_{m-1}P_{m-1}(t),
\end{equation}
associated to the sequence of orthogonal polynomials $(P_{m})_{m \in \mathbb{N}}$ with respect to $\mathrm{d}\omega$. A re-writing of \eqref{eq:OP_reccurence} yields 
...
& \ddots & \ddots & \sqrt{b_{N}} \\
0 & & \sqrt{b_{N}} & a_{N} \end{array} \right) .
\end{equation}


From \eqref{eq:matrix_OP_reccurence}, we conclude that the roots of $P_{N+1}$ are the eigenvalues of the tridiagonal matrix $J_{N}$, and for every root $x$, the vector $\begin{pmatrix}
P_{0}(x)  \\
\vdots \\
P_{N}(x) \\
\end{pmatrix}$ is the corresponding eigenvector. In other words, the spectral decomposition of the corresponding Jacobi matrix gives the nodes and the weights of the Gaussian quadrature. The matrix $J_{N}$ is known explicitly for many classic measures $\mathrm{d}\omega$ such as .... Yet, building up this matrix is not a trivial problem for a general measure $\mathrm{d}\omega$.
