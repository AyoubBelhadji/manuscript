...
As we shall see, the analysis of the integration error in~\eqref{eq:int_error} is convenient if $f$ belongs to an RKHS.
An RKHS over $\mathcal{X}$ is a Hilbert space $\mathcal{F}$ that satisfies the following continuity property: for every $x \in \mathcal{X}$, the evaluation function $f \mapsto f(x)$ is continuous with respect to the norm of $\mathcal{F}$: 

\begin{equation}
\forall x \in \mathcal{X}, \: \exists M_{x} >0 , \: \forall f \in \mathcal{F}, \: |f(x)| \leq M_{x} \|f\|_{\mathcal{F}}.
\end{equation}

This guarantees [??]that for every $x \in \mathcal{X}$ there exists an element $k_{x} \in \mathcal{F}$ such that
\begin{equation}
\forall f \in \mathcal{F}, \: f(x) = \langle f, k_{x} \rangle_{\mathcal{F}}.
\end{equation}
...
& = \|\mu_{g}\|_{\mathcal{F}}^{2} - 2 \bm{w}^{\Tran} \mu_{g}(\bm{x}) + \bm{w}^{\Tran} \bm{K}(\bm{x})\bm{w},
\end{align}

where $\bm{K}(\bm{x}) = (k(x_{n},x_{n'}))_{(n,n') \in [N] \times [N]} \in \mathbb{R}^{N \times N}$ and $\mu_{g}(\bm{x}) = (\mu_{g}(x_{n}))_{n \in [N]} \in \mathbb{R}^{N}$.


In order to prove that \eqref{eq:wce_qmc_2} is a consequence of \eqref{eq:wce_qmc}, it is sufficient to observe that

\begin{align}
\|\mu_{g}\|_{\mathcal{F}}^{2} & = 
\|\bm{\Sigma} g\|_{\mathcal{F}}^{2}\\
...
We can see that  is an instantiation of~\eqref{eq:wce_qmc} by taking .
...
\end{theorem}
Theorem~\ref{thm:Mercer_for_compact} was first proven when $\mathcal{X} = [0,1]$ and $\mathrm{d}\omega$ is the Lebesgue measure in \cite{Mer1909}. A modern proof can be found in \cite{Lax02}, while the proof in the general case can be found in \cite{CuZh07}.  Note, however, that the compactness assumption in Theorem~\ref{thm:Mercer_for_compact} excludes kernels such as the Gaussian or the Laplace kernels.
