...
We start by the problem of linear regression under constraint on the number of observations.
...
In some situations, calculating $\bm{w}^{*}$ using the pseudo-inverse formula is not affordable, either because $N$ is very large and the computation resources are not sufficient (the numerical issue), or because obtaining the $N$ labels $(y_{n})_{n \in [N]}$ is expensive (the sensing issue). In such a situation, looking for repsentatives can be beneficial.

 Formally, a family of representatives of $\bm{X}$ 
 % corresponds to a family of indices $(n_{i})_{i \in I}$ and a family of weights $(s_{i})_{i \in I}$ $...$
 corresponds to a diagonal matrix $\bm{S} \in \mathbb{R}^{N \times N}$ defined by $\bm{S}_{n,n}  = s_{n}$ for every $n \in [N]$. Intuitively, the support of $\bm{S}$ i.e. the set $\{ n \in [N], s_{n} \neq 0 \}$ corresponds to the rows of $\bm{X}$ that will be kept; and the values of the $s_{n}$ corresponds to either the multiplicities of these rows, or some rescaling factors necessary for the theoretical analysis. 
...
(1-\epsilon) \mathbb{I}_{d} \prec \sum\limits_{n \in [N]} s_{n} \tilde{x}_{n} \tilde{x}_{n}^{\Tran} \prec (1+\epsilon) \mathbb{I}_{d} \iff \|\mathbb{I}_{d} - \sum\limits_{n \in [N]} s_{n} \tilde{x}_{n} \tilde{x}_{n}^{\Tran}\|_{2} \leq \epsilon,
\end{equation}
% with
% \begin{equation}
...
% \end{equation}
Therefore, the subsampling problem boils down to looking for the $s_{n}$ that make the reduced matrix $\sum_{n \in [N]} s_{n} \tilde{x}_{n} \tilde{x}_{n}^{\Tran}$ close to the identity matrix $\mathbb{I}_{d}$, which writes as the sum of rank one matrices
...
The notion of leverage scores appears in other subsampling problems such as sparsification of graphs \citep*{SpTe04,SpSr11}, signal recovery on graphs \citep*{PuTrGrVa18}, or low rank approximations \citep*{DrMaMu07} that we will study more in detail in Chapter~\ref{chapter:cssp}.
...
Another field of application of matrix sub-sampling is low rank approximation of kernel matrices that are recurrently used in kernel methods \citep{Bac13,AlMa15}. For example, consider some elements $x_{1}, \dots, x_{N}$ of a metric space $\X$ equipped with a p.s.d. kernel $k: \X \times \X \rightarrow \mathbb{R}$, and consider the kernel ridge regression problem
...
Again, the solution of~\eqref{eq:kernel_ridge_regression_problem} have a tractable formula
\begin{equation}
\bm{\alpha}^{*} = (\bm{K}(\bm{x}) +N\lambda \mathbb{I}_{N})^{-1} \bm{y},
\end{equation}
yet it is not amenable to use for large values of $N$ because the required number of operations to invert $\bm{K}(\bm{x}) +N\lambda \mathbb{I}_{N}$ scales as $\mathcal{O}(N^{3})$. Scaling up kernel methods motivated the interest on low rank approximations of kernel matrices. Indeed, if a low rank approximation of $\bm{K}(\bm{x})$ is known then ..... Nystrom approximation is widely used for its simplicity and because it is amenable to theoretical analysis. This approximation is based on a subset $S \subset [N]$, and writes
\begin{equation}
\bm{K}(\bm{x}) \approx \bm{K}(\bm{x})_{:,S} \bm{K}(\bm{x})_{S,S}^{+} \bm{K}(\bm{x})_{S,:}.
\end{equation}

The theoretical analysis of the Nystrom approximation, for kernel ridge regression, was carried out in \citep{Bac13} under uniform sub-sampling, and in \citep{AlMa15} under the 
...
% In particular, the minimal budget to conducted the Nystrom approximation scales as $\mathcal{O}(d_{\eff}(\lambda) \log d_{\eff}(\lambda))$:
 that plays the same role as the dimension $d$ for linear regression on budget: the minimal budget to conducted the Nystrom approximation scales as $\mathcal{O}(d_{\eff}(\lambda) \log d_{\eff}(\lambda))$.
...
As one would observe, all the problems we reviewed previously are \emph{fixed design} problems, in the sense that it is required to sub-sample from a given set $\{x_{1}, \dots, x_{N}\}$. This setting is typical in numerical linear algebra and machine learning applications where $\{x_{1}, \dots, x_{N}\}$ corresponds to some fixed dataset.


Yet, as it was shown by \cite{Bac17}, the analysis of some \emph{random design} problems can be conducted using the same spectral techniques used for fixed design problems. Indeed, the author considered the problem of approximating a function $f$ defined on some metric space $\X$ by a finite mixture of some kernel translates $k(x_{n},.)$ 
\begin{equation}
f \approx \sum\limits_{n \in [N]}w_{n}k(x_{n},.).
\end{equation}
% where the \emph{nodes} $x_{n}$ are taken to be random in $\X$.

More precisely, $\X$ is assumed to be a measurable space equipped with a measure $\omega$, and $f$ is assumed to writes as
\begin{equation}
f = \int_{\X}k(x,.)g(x) \mathrm{d}\omega(x),
\end{equation}
with $g \in \mathbb{L}_{2}(\mathrm{d}\omega)$. Moreover, the $x_{n}$ are assumed to be independent random variables that follow a distribution $q \mathrm{d}\omega$ and the weights $(w_{n})_{n \in N}$ are taken to minimize 
\begin{equation}
\|f - \sum\limits_{n \in [N]} \frac{w_{n}}{q(x_{n})^{1/2}}k(x_{n},.) \|_{\F}^{2} + N \lambda \|\bm{w}\|^{2}.
\end{equation}
This optimization problem admits a unique solution 
\begin{equation}
\bm{w}^{*} = (\bm{K}(\bm{x})+N\lambda \mathbb{I}_{N})^{-1}f(\bm{x}),
\end{equation}
where $f(\bm{x}) = (f(x_{n}))_{n \in [N]} \in \mathbb{R}^{N}$. 

Moreover, many typical notions of matrix sub-sampling extend to this problem. For example, there exists a distribution $q_{*}$ called the continuous ridge leverage score distribution for which 
\begin{equation}
...
\section{High order sampling: determinantal sampling}
As we have seen in the previous section, independent sampling was used in a variety of approximation tasks that have a linear structure. These sub-sampling methods relies on first-order information such as the leverage score distribution. 
