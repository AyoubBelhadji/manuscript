...
Many problems in applied mathematics can be reformulated as a problem of sensing under  budget constraints.
(3 pages)
...
In [Warnock72], the author derived a tractable formula for the $\mathbb{L}_{2}$-discrepancy:
...
This situation is to compared to the star discrepancy for witch no tractable formula is known: the computation of the star discrepancy is an NP-hard problem [Gnewuch, Srivastav and Winzen].
...
\begin{figure}[]
    \centering
\includegraphics[width= 0.45\textwidth]{img/Sobolev/Bernoulli_kernels_same_node_s_1_5.pdf}
...
This idea can be tracked to Hickernell [??] who introduced reproducing kernels to the QMC community. Indeed, in [??] the author derived a tractable formula for the worst-case integration error of a quasi-Monte Carlo rule:
...
\end{equation}
he ended up by proving that \eqref{eq:wce_qmc} is equal to 
\begin{equation}\label{eq:wce_qmc_2}
 \int_{[0,1]^{d}}\int_{[0,1]^{d}} k(\bm{x},\bm{y}) \mathrm{d}\bm{x} \mathrm{d}\bm{y} - \frac{1}{N}\sum\limits_{n \in [N]} \int_{[0,1]^{d}} k(x_{n},\bm{y}) \mathrm{d} \bm{y} + \frac{1}{N^2}\sum\limits_{n,n'=1}k(x_{n},k_{n'}).
\end{equation}

We can see that \eqref{eq:wce_qmc_2} is an instantiation of .... by taking $g = 1$ and $\X=[0,1]^{d}$, and the $w_{n}= 1/N $.

By considering the kernel $k_{QMC}$ defined by
...
so that $\mathcal{F}=\mathcal{F}_{s}$ is the Sobolev space of order $s$ on $[0,1]$. Note that $k_{s}$ can be expressed in closed form using Bernoulli polynomials \cite{Wah90}:
\begin{equation}
...
In \cite{Bac17}, the author proposed to sample the nodes $(x_j)$ i.i.d. from some proposal distribution $q$, and then pick weights $\hat{\bm{w}}$ in \eqref{e:quadrature} that solve the optimization problem
...
\end{equation}
for some regularization parameter $\lambda>0$. The idea of the author was to make use of the spectral decomposition of the integral operator $\bm{\Sigma}: \mathbb{L}_{2}(\mathrm{d}\omega) \rightarrow \mathbb{L}_{2}(\mathrm{d}\omega)$ defined by
\begin{equation}
\bm{\Sigma} f (\cdot) = \int_{\mathcal{X}} k(\cdot,y)f(y) \mathrm{d}\omega(y), \quad f \in \mathbb{L}_{2}(\mathrm{d}\omega),
\end{equation}
in the design of the quadrature. Finally, we note that Proposition~\ref{p:bach} highlights the fundamental role played by the spectral decomposition of the operator $\bm{\Sigma}$ in designing and analyzing kernel quadrature rules.
 

In the analysis in \cite{Bac17}, any proposal distribution $q$ could be used to sample $N$ i.i.d. nodes that constitutes the design $\bm{x}$. Indeed, the author proved the following result.
...
\end{equation}
Indeed, for this specific choice 
\begin{align}
...
\begin{corollary}
  Let $\bm{x} = \{x_{1}, \dots , x_{N}\}$ be a projection DPP with reference measure $\mathrm{d}\omega$ and kernel \eqref{e:kernel}. Then $\bm{K}(\bm{x})$ is a.s. invertible, so that \eqref{eq:unreg_opt_problem} has unique solution $\hat{\bm{w}} = \bm{K}(\bm{x})^{-1}\mu_{g}(x_{j})_{j \in [N]}$ a.s.
...
DPPs allow closed form expressions for the expectation of trigonometric functions of such angles; see \cite{BeBaCh18Sub} and Appendix~\ref{app:geometric_intuition} for the geometric intuition behind the proof.
The second step thus consists in developing the expectation of the bound under the DPP.
...
The eigenvalues of some random matrix models corresponds to some specific projection DPPs. 
...
Consider $(\psi_{m})_{m \in \mathbb{N}^{*}}$ an orthonormal family with respect to the measure $\mathrm{d}\omega$. In \citep*{HoKrPeVi06}, a sequential algorithm was proposed for the numerical simulation of a random set that follows the distribution of the projection DPP of marginal kernel
\begin{equation}
\KDPP_{N}(x,y) = \sum\limits_{n \in [N]} \psi_{n}(x)\psi_{n}(y).
\end{equation}
 
Using the chain rule, this density factorises as the product of $N$ conditionals
\begin{equation}
f_{\KDPP}(x_{1},\dots,x_{N}) = \prod\limits_{n \in [N]} f_{\KDPP}(x_{n} | x_{1}, \dots, x_{n-1}),
\end{equation}
for every $n \in [N]$ the conditional expresses as
\begin{equation}
f_{\KDPP}(x | x_{1} \dots x_{n-1}) = \frac{1}{n} \left( \KDPP_{N}(x,x) - \KDPP_{N}(x,\bm{x})^{\Tran}\bm{\KDPP}(\bm{x})^{-1} \KDPP_{N}(x,\bm{x}) \right),
\end{equation}
where $\KDPP_{N}(x,\bm{x}) = (\KDPP_{N}(x,x_{\ell}))_{\ell \in |\bm{x}|}$.
...
\Aitem $\mu_{N}(x) \longleftarrow  \frac{1}{N} \KDPP_{N}(x,x)\mathrm{d}\omega(x)$
\Aitem $\bm{x} \longleftarrow  \emptyset$
\Aitem \While $n > 0 $
\Aitem \mt  Pick $x_{n}$ from $\mathcal{X}$ from $\mu_{n}$ \ar{Sample from the conditional}
\Aitem \mt $\mu_{n}(x) = \frac{1}{n} \left( \KDPP_{N}(x,x) - \KDPP_{N}(x,\bm{x})^{\Tran}\bm{\KDPP}(\bm{x})^{-1} \KDPP_{N}(x,\bm{x}) \right) \mathrm{d}\omega(x)$ \ar{Chain rule}
...
\caption{Pseudocode of the HKPV algorithm for sampling from a projection DPP of marginal kernel $\KDPP_{N}$.}
\label{alg:HKPV_SAMPLER}
\end{figure}
The authors proved that the Algorithm~\ref{alg:HKPV_SAMPLER} output a random set that follows the distribution of the projection DPP of marginal kernel $\KDPP_{N}$. In practice, the numerical implementation of this algorithm goes through sampling the conditional distributions. This problem was considered in \citep{BaHa16}. 
...
\end{equation}
where the convergence is point-wise on $\mathcal{X}$.
...
\begin{align}
\bm{\Pi}_{\mathcal{T}(\bm{x})}e_{n}^{\mathcal{F}} & =  \sum\limits_{i \in [N]}c_{i}\sum\limits_{m \in \mathbb{N}^{*}} \sigma_{m}e_{m}(x_{i})e_{m}(.)\\
& = \sum\limits_{m \in \mathbb{N}^{*}} \sqrt{\sigma_{m}}\left(\sum\limits_{i \in [N]}c_{i} e_{m}(x_{i}) \right) e_{m}^{\mathcal{F}}(.) \nonumber.
\end{align}
Since $(e_m^{\cal F})_{m\in \mathbb{N}^{*}}$ is orthonormal,
\begin{align}\label{eq:interpolation_error_and_vector_c}
\|\bm{\Pi}_{\mathcal{T}(\bm{x})}e_{n}^{\mathcal{F}}\|^{2}_{\mathcal{F}} & = \sum\limits_{m \in \mathbb{N}^{*}} \sigma_{m} \left(\sum\limits_{i \in [N]}c_{i} e_{m}(x_{i}) \right)^{2}\\
& = \sum\limits_{m \in \mathbb{N}^{*}} \sigma_{m} \sum\limits_{i \in [N]} \sum\limits_{j \in [N]}c_{i}c_{j} e_{m}(x_{i})e_{m}(x_{j}) \nonumber\\
& = \sum\limits_{m \in \mathbb{N}^{*}} \bm{c}^{\Tran} e_{m}^{\mathcal{F}}(\bm{x})e_{m}^{\mathcal{F}}(\bm{x})^{\Tran} \bm{c} \nonumber\\
& = \bm{c}^{\Tran} \sum\limits_{m \in \mathbb{N}^{*}}  e_{m}^{\mathcal{F}}(\bm{x})e_{m}^{\mathcal{F}}(\bm{x})^{\Tran} \bm{c} \nonumber.
\end{align}
Using Mercer's theorem, see \eqref{eq:truncated_kernel_limit},
\begin{equation}\label{eq:matricial_mercer_decomposition}
  \bm{K}(\bm{x}) = \sum\limits_{m \in \mathbb{N}^{*}}  e_{m}^{\mathcal{F}}(\bm{x})e_{m}^{\mathcal{F}}(\bm{x})^{\Tran}.
\end{equation}
Combining \eqref{eq:interpolation_error_and_vector_c} and \eqref{eq:matricial_mercer_decomposition} along with the definition of the vector $\bm{c} = \bm{K}(\bm{x})^{-1}e_{n}^{\mathcal{F}}(\bm{x})$ yields
\begin{align}
\|\bm{\Pi}_{\mathcal{T}(\bm{x})}e_{n}^{\mathcal{F}}\|^{2}_{\mathcal{F}} & = \bm{c}^{\Tran} \bm{K}(\bm{x}) \bm{c}\\
