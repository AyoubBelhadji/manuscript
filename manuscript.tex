...
We start by the case of the Circular Unitary Ensemble (CUE) presented in Section~\ref{eq:DPP_examples}. Remember that this DPP is defined on the unit complex circle $\mathbb{U} = \{ u \in \mathbb{C}, |u| =1 \}$ equipped with Lebesgue measure, and the corresponding kernel:
\begin{equation}
\KDPP(x,y) = \sum\limits_{n \in [N]} e^{2 \pi n \mathbf{i}(x-y)}.
\end{equation}  

the corresponding kernel $\KDPP$  to the DPP : the set of the eigenvalues of a random matrix chosen from Haar measure on the unitary group $\mathbb{U}_{N}$. \cite{Wey46} proved that this random set follows the distribution of a projection DPP defined on 
...
Let $g$ be a real-valued function on $\mathbb{U}$ with $g(e^{\mathbf{i}  \theta}) = \sum_{m =-\infty}^{+\infty} \hat{g}_{m}e^{\mathbf{i}  m \theta}$
 such that $\hat{g}_{0} = 0$ and $ \sum\limits_{m \in \mathbb{N}^{*}} m|\hat{g}_{m}|^{2} <+\infty$ and let $(e^{\mathbf{i} \theta_{n}})_{n \in [N]}$ be the CUE. \\
 Then the random variable $ \frac{1}{N}\sum_{n \in [N]} g(e^{\mathbf{i}\theta_{n}})$
...
converges to $ \mathcal{N}(0,\frac{2}{N}\sum_{m \in \mathbb{N}^{*}} m|\hat{g}_{m}|^{2})$.
\end{theorem}
