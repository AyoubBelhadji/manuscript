...
In \citep{BaHa16}, the authors proved the first CLT for a quadrature based on determinantal point process nodes in a high-dimensional setting. This work deals with quadratures defined on the integration domain $[-1,1]^{d}$ for the approximation of integrals that write
\begin{equation}
\int_{[-1,1]^{d}} f(\bm{u}) \mathrm{d}\omega(\bm{u}),
\end{equation}
where $\mathrm{d}\omega$ is a measure that can be factorized as
\begin{equation}
\mathrm{d}\omega(\bm{u}) = \prod\limits_{\delta \in [d]} \mathrm{d}\omega_{\delta}(\bm{u_{\delta}}),
\end{equation}
where the measures $\mathrm{d}\omega_{\delta}$, defined on $[-1,1]$, satisfy some regularity conditions that we precise in the following.

Let $\mathrm{d}\omega$ a measure on the real line supported on $[-1,1]$. Denote by $(a_{m})_{m \in \mathbb{N}}$ and $(b_{m})_{m \in \mathbb{N}}$ the sequences of coefficients appearing in the three-term recurrence relation:
\begin{equation}
x P_{m}(x) = a_{m}P_{m+1}(x) + b_{m}P_{m}(x) + a_{m-1}P_{m-1}(x),
\end{equation}
associated to the sequence of orthogonal polynomials $(P_{m})_{m \in \mathbb{N}}$ with respect to $\mathrm{d}\omega$.
...
The measure $\mathrm{d}\omega$ is said to be of Nevai class if and only if
\begin{equation}
...
\end{equation}
corresponds to the Chebyshev orthogonal polynomials of the first kind; and the corresponding measure $\mathrm{d}\omega$ admits a density $\omega$ with respect to the Lebesgue measure:
\begin{equation}
\omega(x) = \frac{1}{\sqrt{1-x^{2}}}.
\end{equation}

In other words, the measures belonging to the Nevai class have three terms coefficients that converges to the corresponding coefficients of the measure $\mathrm{d}\omega$.
...
The quadrature defined this way defines an unbiased estimator of the integral $\int_{[0,1]^{d}} f(\bm{u}) \mathrm{d}\bm{u}$ with a variance that depends on the Fourier coefficients of $f$. 
