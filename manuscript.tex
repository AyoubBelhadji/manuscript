...
\end{equation}
\end{theorem}
The Koksma-Hlawka inequalities gives a decoupled upper bound for the integration error: the $\mathbb{L}_{p}$-discrepancy of the configuration $\bm{x}$ does not depend on $f$ and the $q$ variation of the function $f$ does not depend on the configuration $\bm{x}$. This decoupling allows to focus only on the discrepancy of the design when the function $f$ is sufficiently regular.
...
From a distance, a regular lattice is "well-spread" in the hypercube $[0,1]^{d}$ and can be used as the configuration of a quadrature rule. Yet, by considering the star-discrepancy of uniform grids, we reach the counter-intuitive conclusion that the uniform grids are not convenient for numerical integration. Indeed, consider the regular lattice $\bm{x}_{N}  \subset \mathbb{R^{d}}$, where $N = n^{d}$ for some $n \in \mathbb{N}^{*}$, defined by
\begin{equation}
...
The Koksma-Hlawka inequalities suggest that 
...
The construction of these sequences goes as follow: let $b \geq 2$ be an integer; and define for every $n \in \mathbb{N}$ the $b$-adic expansion:
\begin{equation}
...
% \end{definition}
For instance, for $b = 2$, the first elements of the sequence are $\displaystyle 0, \frac{1}{2}, \frac{3}{4}, \frac{1}{8}, \frac{5}{8}, \frac{3}{8}, \dots$. While, for $b=10$, the elements of the sequence goes as follow $\displaystyle 0.1, 0.2, \dots, 0.9, 0.01, 0.11, \dots ,0.91, \dots$.
...
\end{equation}
These are the Halton sequences. Another possible construction is the Hammersley configuration defined for a fixed value of $N$:
\begin{equation}
x_{n} = ((n-1)/N,v_{b_1,n}, \dots, v_{b_{d-1},n}).
\end{equation}
These cryptic configurations have an interesting property: they are low discrepancy sequences on $[0,1]^{d}$. Indeed, under the assumption that the $b_{1}, \dots, b_{d}$ are pairwise relatively prime, the star discrepancy of the Halton sequence is $\displaystyle \mathcal{O}\bigg(\frac{(\log N)^{d}}{N} \bigg)$; while the star discrepancy of the Hammersley sequence is $\displaystyle \mathcal{O} \bigg(\frac{(\log N)^{d-1}}{N} \bigg)$. This is to be compared to the rate of the regular lattice that scales as $\mathcal{O}(N^{-1/d})$.
