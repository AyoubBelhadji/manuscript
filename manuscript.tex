...
where
\begin{equation}
\sigma_{m}^{\mathcal{G}_{\gamma}} = \sqrt{\frac{2a}{A}} B^{m}, \quad e_{m}^{\mathcal{G}_{\gamma}}(x) = e^{-(c-a)x^{2}}H_{m}(\sqrt{2c}x),
\end{equation}
...
\subsection{DPP based quadratures that achieve the Bakhvalov lower bound?}
