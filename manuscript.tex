...
We give now the definition of a DPP. First, we consider a measurable function $\kappa: \Omega \times \Omega \rightarrow \mathbb{C}$.
\begin{definition}
Let $\gamma$ be a simple point process on $\Omega$. $\gamma$ is said to be a determinantal point process with kernel $\kappa$ and reference measure $\mathrm{d}\omega$ if its joint intensities with respect to the measure $\mathrm{d}\omega$ satisfies
\begin{equation}\label{eq:correlation_functions_dpp_def}
\forall x_{1}, \dots, x_{m} \in \Omega, \:\: \rho_{m}(x_{1}, \dots, x_{m}) = \Det \bm{\kappa}(\{ x_{1}, \dots, x_{m} \}),
\end{equation}
for every $m \in \mathbb{N}^{*}$.
\end{definition}

The existence of such a simple point process is conditioned by some properties that $\kappa$ should satisfy. First, since the correlation functions are positive functions a.s. 
\begin{equation}\label{eq:positivitiy_of_dets_condition}
\Det \bm{\kappa}(\{x_{1}, \dots, x_{m}\}) \geq 0, \text{a.s.} (\mathrm{d}\omega^{\otimes m}) .
\end{equation}
The constraint \eqref{eq:positivitiy_of_dets_condition} is satisfied whenever $\kappa$ is Hermitian:
\begin{equation} 
\kappa(x,y) = \overline{\kappa(y,x)}, \: \text{a.s.}  (\mathrm{d}\omega \times \mathrm{d}\omega),
\end{equation}
and positive:
\begin{equation}
\forall a_{1}, \dots, a_{M} \in \mathbb{C}, \:\: \sum\limits_{m,m' \in [M]} a_{m}\overline{a_{m'}} \kappa(x_{m},x_{m'}) \geq 0.
\end{equation} 
...
The conditions assumed previously are still not enough to guarantee the existence of a simple point process that satisfies~\eqref{eq:correlation_functions_dpp_def}. We make a further assumption: $\kappa$ is is locally square integrable on $\Omega^{2}$, that is for any compact set $C$ of $\Omega$, we have
\begin{equation}
\int_{C^{2}} |\kappa(x,y)|^{2} \mathrm{d}\omega(x) \mathrm{d}\omega(y) < +\infty.
\end{equation}

 yet they are enough to define an operator with interesting properties: the corresponding integration operator with respect to $\mathrm{d}\omega$
% \begin{equation}
% \bm{\Sigma}_{\kappa}: .
% \end{equation}
...
Indeed, $\bm{\Sigma}_{\kappa}$ is a non-negative definite self-adjoint integral operator on $\mathbb{L}_{2}(\mathrm{d}\omega)$.



\begin{theorem} [\cite{Mac75,Sos00}]
Let $\kappa$ a kernel such that the integral operator $\bm{\Sigma}_{\kappa}$ is self-adjoint and locally trace class. 

Then $\kappa$ defines a DPP on $\Omega$ if and only if the spectrum of $\bm{\Sigma}_{\kappa}$ is contained in $[0,1]$.
\end{theorem}
...
\end{example}
A DPP defined using a projection kernel, like in Example~\ref{ex:projection_dpp}, is called a projection DPP. These DPPs are important ... They define DPPs with deterministic cardinalities.
\begin{proposition}
Let $v_{1}, \dots, v_{M} \in \mathbb{L}_{2}(\mathrm{d}\omega)$, such that $(v_{m})_{m \in [M]}$ is an orthonormal family of $\mathbb{L}_{2}(\mathrm{d}\omega)$, and define the kernel
\begin{equation}
\kappa(x,y) = \sum\limits_{m \in [M]}v_{m}(x)\overline{v_{m}(y)}. 
\end{equation} 
Then $\gamma$ has $M$ points a.s.

\end{proposition}

\begin{theorem}
Let $\gamma$ be a DPP with a kernel $\kappa$ satisfying the usual conditions.

Define $(\sigma_{m},v_{m})$ the eigenpairs of the integration operator $\bm{\Sigma}_{\kappa}$ with $\sigma_{m} \in [0,1]$.
...
Then $\gamma_{I}$ is determinantal point process associated to the kernel $\kappa$.
\end{theorem}
