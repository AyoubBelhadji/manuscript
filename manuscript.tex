...
Until now, these randomized sub-sampling techniques were based on independent random variables that may coincide making the sampling non optimal. One may expect to improve the sub-sampling efficiency by using a randomized averaging that avoids redundancy and allows to capture the essential of the information.
...
In order to situate the contributions of this thesis, a brief review on randomized subsampling techniques based on linear algebra and functional analysis is given in the following.
...
Polish spaces are recurrently used in probability theory. We give some examples of these abstract spaces.
...
Now, if an element $x_{n}$ is repeated more than once in the collection, $\mu$ is no longer simple and it can be seen as a discrete subset of $\mathcal{D}$ with multiplicities. Yet, we are mostly interested in simple counting measures in this manuscript.

Now, we can define properly a point process in $\mathcal{D}$. It is a special case of a random measure that we recall the definition in the following. 
...
\end{example}
We shall see later a similar characterization of the intensity function of simple point processes in the case of a continuous domain.

Now, given a simple point process $\gamma$, the identity~\eqref{eq:first_intensity_measure} can be rewritten (explain why: a simple counting measure is atomic in polish space????)
...
We illustrate  the \emph{negative correlation} property of DPPs by a comparison with Poisson point processes. Consider $\gamma$ be a DPP of kernel $\kappa$ and reference measure $\mathrm{d}\omega$. We assume that $\kappa$ satisfies the usual conditions, and denote by $\gamma_1$ its mean measure. Consider $\tilde{\gamma}$ a Poisson point process of intensity $\gamma_{1}$. 
...
\subsubsection{An example: periodic Sobolev spaces}
...
\end{equation}
Figure~\ref{fig:periodic_sobolev_kernel} illustrates the graphs of the kernel translates $x \mapsto k_{s}(0.3,x)$ for $s \in \{1, 2,3\}$. The elements of $\mathcal{F}_{s}$ are periodic and are characterized by their Fourier series coefficients (see Example 19, Chapter 7 in \citep{BeTh11})
\begin{equation}
...
\end{equation}
The corresponding RKHS $\mathcal{F}=\mathcal{F}_{s}$ is the periodic Sobolev space of order $s$: an element of $\mathcal{F}_{s}$ is a function $f$ defined on $[0,1]$, that have the derivative of order $s$ defined in the sense of distributions such that
...
\end{align*}
  is an isomorphism of Hilbert spaces between the closed span of $\{ e_{m}; \sigma_m>0\}$ in $\mathbb{L}_{2}(\mathrm{d}\omega)$ and $\mathcal{F} \subset \mathbb{L}_{2}(\mathrm{d}\omega)$. In particular, for every $f \in \F$, there exists $g \in \mathbb{L}_{2}(\mathrm{d}\omega)$ such that
  \begin{equation}
  f = \bm{\Sigma}^{1/2}g,
  \end{equation}
  with $\|f\|_{\mathcal{F}} = \|g\|_{\mathrm{d}\omega}$. Since $\mathrm{d}\omega$ is supposed to be non-degenerate, $\mathcal{F}$ is dense in $\mathbb{L}_{2}(\mathrm{d}\omega)$ if and only if all the eigenvalues $\sigma_{m}$ are positive. Under this condition, $\bm{\Sigma}^{1/2}$ is an isomorphism between $\mathbb{L}_{2}(\mathrm{d}\omega)$ and $\F$. Finally, note that 
  \begin{equation}
...
\end{equation}
Note that this condition is not necessary (Example 2.9, \citep{StSc12}). Now, under the compact embedding assumption, the pointwise convergence of the Mercer decomposition to the kernel $k$ is equivalent to the injectivity of the embedding $I_{\mathcal{F}}$ (Theorem 3.1, \citep{StSc12}). The injectivity of $I_{\mathcal{F}}$ is guaranteed whenever $\mathrm{d}\omega$ is non-degenerate (Chapter 4 in \citep{StCh08}). 

The condition~\eqref{eq:condition_integrability_diagonal} is satisfied by the Gaussian kernel on $\mathbb{R}^{d}$ equipped with the Gaussian measure.
...
\subsubsection{The Korobov spaces}
